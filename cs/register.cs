...
//		PrintBasicHeader();
	...
	if(Request.QueryString["a"] == "reg_select")
	{
		Response.Write(ReadSitePage("public_body_register"));
		return;
	}
	/*if(!TS_UserLoggedIn() && Session["terms_read"] == null)
	{
		...
		Response.Write("<center><table width=100% cellpadding=0 cellspacing=0 border=0><tr>");
...
void GetQueryString()
{
	...
	string reg = "";
	if(Request.QueryString["a"] == "reseller_reg")
		reg = "dealer";
	if(Request.QueryString["a"] == "customer_reg")
		reg = "customer";
	string sc = "";
	...
	else if(action == "updated")
	{
		...
		Response.Write("<br><input type=button " + Session["button_style"].ToString() + " onclick=window.location=('");
		Response.Write("c.aspx') value='Main Page'><br><br><br>\r\n");
	}
	else
	{
		//DEBUG("reg==",reg);
...
Boolean ValidateUserDetails()
{
    ...
    string vaildJson=validRobot(EncodedResponse);
	Trim(ref email);
	Trim(ref email_old);
...
//  dr["language"] = "1";
    ...
    if (vaildJson.IndexOf("true")==-1)
        err = "You did not pass the robot validation.";
   ...
   //	else if(!TS_UserLoggedIn() && Request.Form["password"].ToString() == "" && Request.Form["type"].ToString() != "2")
	//	err = "Error, Password can't be blank.";
	//else if(!TS_UserLoggedIn() && Request.Form["password"].ToString() != Request.Form["confirm_password"].ToString() && Request.Form["type"].ToString() != "2")
	//	err = "Error, Your passwords don't match.";
...
//	Response.Write("</td>");
//	Response.Write("<td width=10></td>");
	Response.Write ("<td style='border:1px solid #e1e1e1;'>");
	Response.Write("<form action=register.aspx?action=validate method=post>");
	DrawCardTable(dr, bNew, reg); //draw empty
...
Boolean CreateAccount()
{
	...
	string type = Request.Form["type"];//GetEnumID("card_type", "customer");
...
//	string country = Request.Form["country"];
	...
	bool bRet = AddNewDealer(email, password, trading_name, corp_number, directory, gst_rate, 
		...
		sm_ddi, sm_mobile, ap_name, ap_email, ap_ddi, ap_mobile, "", type, "", "1");
	...
	if(bRet)
	{
		...
		msgMail.Body += "<br>\r\nCompany : " + company + "<br>\r\n";
		msgMail.Body += "Account Name : " + name + "<br>\r\n";
		msgMail.Body += "Your EMail : " + email + "<br><br>\r\n";
...
{
	int rows = 0;
	string sc = " SELECT c.*, c1.trading_name AS branch_name, a.class_name, '1' as language ";
...
{
	int rows = 0;
	string sc = " SELECT *, '1' as language FROM card ";
