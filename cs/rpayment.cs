...
string m_branchID = "";
string m_type = "";
string m_tableTitle = "Payment Summary";
string m_datePeriod = "";
string m_dateSql = "";
string m_code = "";
DataRow[] m_dra = null;
...
//				string file_id = file + "_" + (f.Length/1000).ToString() + "K_" + f.LastWriteTime.ToString(date_format);
			...
			if(!m_bPrint)
			{
				LFooter.Text = "<br><br><center><a href=rpayment.aspx";
...
void PrintMainPage()
{
	...
	Response.Write("<form name=f action='rpayment.aspx?r="+ DateTime.Now.ToOADate() +"");
	Response.Write("' method=post>");

	Response.Write("<br><center><h3>Payment Report</h3>");
...
bool DoPaymentSummary(string status)
{
	...
	sc += " SELECT DISTINCT  c.id AS custID, c.company, c.name AS Customer, d.trans_date AS trans_date, i.commit_date AS invoice_date, t.amount";//
	sc += " , ti.amount_applied AS amount2 ";
	sc += " , ISNULL(e.name, (SELECT DISTINCT ee.name FROM tran_detail tdd JOIN ";
	sc += " enum ee ON ee.id = tdd.payment_method AND ee.class = 'payment_method' WHERE tdd.id = ti.id)) AS name ";
//	sc += " , e.name AS name ";
	sc += ", i.invoice_number, i.total ";
//	sc += ", c.name AS customer, c.id AS customer_id ";
	
/*	sc += " FROM invoice i ";
	sc += " LEFT OUTER JOIN tran_invoice ti ON ti.invoice_number = i.invoice_number AND ti.purchase = 0 ";
	sc += " LEFT OUTER JOIN tran_detail d ON d.invoice_number = RTRIM(LTRIM(STR(i.invoice_number))) ";
//	sc += " OR d.id = ti.tran_id ";
	sc += " LEFT OUTER JOIN trans t ON t.id = d.id ";
*/
	sc += " FROM tran_detail d JOIN ";
	sc += " tran_invoice ti ON ti.tran_id = d.id AND ti.purchase = 0 JOIN ";
	sc += " trans t ON t.id = d.id AND t.id = ti.tran_id  JOIN ";
    sc += " invoice i ON i.invoice_number = ti.invoice_number  JOIN ";
	sc += " branch br ON br.id = i.branch AND br.activated = 1 JOIN ";
    sc += " enum e ON e.id = d.payment_method AND e.class = 'payment_method' INNER JOIN ";
    sc += " card c ON c.id = i.card_id ";

//	sc += " JOIN invoice i ON i.invoice_number = d.invoice_number ";
	sc += " WHERE 1=1 ";
	sc += m_dateSql;	
	if(m_customerID != null && m_customerID != "" && m_customerID != "all")
		sc += " AND c.id = "+ m_customerID;
	if(m_paymentType != "")
		sc += " AND d.payment_method = " + m_paymentType;
	if(Session["branch_support"] != null && m_branchID != "" && m_branchID != "0")
	{
		sc += " AND i.branch = "+ m_branchID;
	}
////get all unpaid invoice
	sc += " UNION SELECT c.id AS custID, c.company, c.name AS Customer, isnull(d.trans_date, i.commit_date) AS trans_date, i.commit_date AS invoice_date, t.amount, t.amount AS amount2, e.name, i.invoice_number, i.total";
	sc += " FROM invoice i LEFT OUTER JOIN tran_detail d ON d.invoice_number = RTRIM(LTRIM(STR(i.invoice_number))) ";
	sc += " JOIN branch br ON br.id = i.branch AND br.activated = 1 ";
	sc += " LEFT OUTER JOIN trans t ON t.id = d.id ";
...
//	sc += invSQL;
	...
	//get customer credit with us
	sc += " UNION SELECT c.id AS custID, c.company, c.name AS Customer, d.trans_date AS trans_date, GETDATE() AS invoice_date, ct.amount - ct.amount_applied AS amount ";
	sc += " , ct.amount - ct.amount_applied AS amount2 ";
	sc += ", e.name AS name, '0' AS invoice_number ";
	sc += " , ct.amount AS total ";
//	sc += ", c.name AS customer, c.id AS customer_id ";
	sc += " FROM credit ct  ";
	sc += " JOIN branch br ON br.id = ct.branch AND br.activated = 1 ";
	sc += " JOIN tran_detail d ON d.id = ct.tran_id ";
	sc += " JOIN card c ON c.id = d.card_id ";
	sc += " LEFT OUTER JOIN enum e ON e.id = d.payment_method AND e.class = 'payment_method' ";
	sc += " WHERE ct.amount-ct.amount_applied <> 0 ";
	if(m_paymentType != "")  
		sc += " AND d.payment_method = " + m_paymentType;
	sc += trandsSQL;
	if(m_customerID != null && m_customerID != "" && m_customerID != "all")
...
{
	string sBranchName = "";
	string enableEftpos = GetSiteSettings("enable_DPS_Payment");
	if(enableEftpos == "1")
	 m_enableEftpos = true;
	 
	if(Session["branch_support"] != null)
		sBranchName = GetBranchName(m_branchID);
	if(sBranchName == "")
		sBranchName = "ALL";
	...
	Response.Write("<input type=hidden name=hCustomerID value="+ m_customerID +">");
	Response.Write("<center><table width=100%>");
	...
	Response.Write("Payment Report</font>");
	if(m_paymentType != "")
...
//	Response.Write("<th align=right>Credit</th>");
	...
	if(m_enableEftpos)
	Response.Write("<th align=right>Merch Number</th>");
	Response.Write("</tr>");
	if(rows <= 0)
	{
		Response.Write("</table>");
	...
	//	customer = dr["customer"].ToString();
		...
		if(inv != inv_old)
		{
			if(inv_old != "")
			{
				...
				if(m_enableEftpos)			
		        merchNumber= getMerchNumber(inv_old);
...
//				Response.Write("<td align=right>" + (dCredit==0 ? "" : dCredit.ToString("c")) + "</td>");
				...
				if(m_enableEftpos)
				Response.Write("<td align=right><a href=# onclick=\"window.open('cheftplog.aspx?r=i&inv="+inv_old+"','new', 'width=400, height=500')\">"+ merchNumber +"</a></td>");
				Response.Write("</tr>");
			}
...
//	Response.Write("<td align=right>" + (dCredit==0 ? "" : dCredit.ToString("c")) + "</td>");
	...
	Response.Write("<td align=right>" + dPaymentTotal.ToString("c") + "</td>");
	if(m_enableEftpos)
	Response.Write("<td align=right><a href=# onclick=\"window.open('cheftplog.aspx?t=i&inv="+inv_old+"','new', 'width=400, height=500')\">"+ merchNumber +"</a></td>");
	Response.Write("</tr>");
...
//	Response.Write("<th align=right>Credit</th>");
	...
	if(!m_bPrint)
	{
		...
		Response.Write(" onclick=\"window.open('rpayment.aspx?pm=" + m_paymentType + "&print=1&pr="+ m_nPeriod +"");
		if(m_sdFrom != null)
...
string getMerchNumber(string invoice)
{
	if(ds.Tables["merch"] != null)
		ds.Tables["merch"].Clear();

	string sc = " SELECT * FROM eftposlog WHERE inv_number = " + invoice;
	try
	{
		myAdapter = new SqlDataAdapter(sc, myConnection);
		if(myAdapter.Fill(ds, "merch") == 1)
			return ds.Tables["merch"].Rows[0]["merch_number"].ToString();
	}
	catch(Exception e) 
	{
	  if(e.ToString().IndexOf("Invalid object name 'eftposlog'") >= 0)
	  {
	  	
      	sc = @"
			CREATE TABLE [dbo].[eftposlog](
			[inv_number][int]  NULL,
			[merch_number][varchar](50) COLLATE SQL_Latin1_General_CP1_CI_AS  NULL,
			[datetime][datetime]  NULL,
			[amount][money] NULL,
			[accept][int] NULL,
			[sales_barcode][varchar](50) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
			[payment_id][int]NULL,
			[branchId] [int] NULL
	 		)";
	  try
