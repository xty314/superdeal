...
string m_itemCols = "9"; //item table columns
...
string BuildInvoice(string sInvoiceNumber)
{
	...
	sc += " i.freight, i.sales_note, c.*, s.code, s.supplier_code, s.quantity, s.name as item_name, cr.brand, cr.price1, i.branch ";
...
//DEBUG("bysteym = ", bSystem.ToString());
	...
	/*sb.Append("<td width=10%><b>CODE</b></td>\r\n");*/
	sb.Append("<td width=10%><b>Box#</b></td>\r\n");
	sb.Append("<td width=10%><b>Branch</b></td>\r\n");
	sb.Append("<td width=12%><b>M_PN#</td>\r\n");
	sb.Append("<td width=13%><b>Picture</b></td>\r\n");
	sb.Append("<td width=30%><b>DESCRIPTION</td>\r\n");
...
//	if(bSystem && bIndividual_Price)
	...
	{		
		...
		sb.Append("<td width=9% align=right><b>QTY</td>\r\n");
		sb.Append("<td width=15% align=right>&nbsp;</td></tr>\r\n");
	}
	else
	{   
		/*sb.Append("<td width=8% align=right><b>DISC(%)</td>\r\n");*/
		sb.Append("<td width=12% align=right><b>SELLING PRICE</td>\r\n");
		sb.Append("<td width=9% align=right><b>QTY</td>\r\n");
		sb.Append("<td width=15% align=right><b>AMOUNT</td></tr>\r\n");
	}
...
//DEBUG("rows="+dt.Rows.Count, " j="+j);
	...
	if(bOrder)
	{
		...
		double dGST = MyDoubleParse(dt.Rows[0]["customer_gst"].ToString());	//MyDoubleParse(GetSiteSettings("gst_rate_percent", "12.5")) / 100; //Get GST rate for Site Sitting //Modified by NEO
...
//	string m_sKitTerm = GetSiteSettings("package_bundle_kit_name", "Kit", true);
  ...
  // if(m_bKit || bKit)
   ...
   double dNormalPrice = double.Parse(dr["normal_price"].ToString(), NumberStyles.Currency, null); 
	  dNormalPrice = Math.Round((dNormalPrice/1.125),2);	 // Exclu G.S.T  
...
//DEBUG("Commit_Price", dPrice);
//	double dTotal = (dPrice * (1-dDiscountPercent)) * quantity * 1.125; // don't use the currency format to get the correct total price...
...
//	sb.Append("<td valign='top'>"+ m_nItemCounter +". </td>");
	...
	{
		sb.Append("<td valign=top>" + dr["code"].ToString() +"</td>");
		sb.Append("<td valign=top>" +branch + "</td>");
	
		sb.Append("<td valign=top>" + dr["supplier_code"].ToString() + "</td>");
		sb.Append("<td valign=top>" );
		
		sb.Append("<img width=50 src='" + src + "' border=0 >");
		sb.Append("</td>");
//		sb.Append("<td>" + dr["brand"].ToString() + " " + dr["item_name"].ToString());
		sb.Append("<td valign=top>" + dr["item_name"].ToString() +"</td>");
	}
	
		 	
	// sb.Append("<td width=10>"+dNormalPrice.ToString("c")+"</td>"); // ssssssssssssssssssssssssssssssssssssss
	/*sb.Append("<td align=right width=5%>");
	if(dDiscountPercent > 0 || dDiscountPercent < 0)
		sb.Append(dDiscountPercent.ToString("p"));
	sb.Append("</td>"); */
	sb.Append("<td width=12% align=right valign=top>");
...
//	if((bSystem && bIndividual_Price) || bKit)
	...
	else
	{
		...
		/*
		   ...
		   // if(bIncludeGST)
		//	dcprice = Math.Round(dcprice * 1.125, 2);
		  // Exclu G.S.T
		...
		*/
			...
			else{ // if discount not equal null
	 ...
	 //  	DEBUG("Isellingprice ", Isellingprice);
			    }else{
				sb.Append(MyDoubleParse(dr["commit_price"].ToString()).ToString("c"));	
	...
	if(!bKit)
	{
		if(m_bDisplayServiceItemWithNoPrice)
		{
			if(bool.Parse(dr["is_service"].ToString()) && double.Parse(dr["commit_price"].ToString()) == 0)
				...
				sb.Append("<td width=5% align=right valign=top>" + dr["quantity"].ToString() + "</td>");
		}
		else
			sb.Append("<td width=5% align=right valign=top>" + dr["quantity"].ToString() + "</td>");
	}
