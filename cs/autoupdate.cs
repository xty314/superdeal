...
int nsize = (int)Math.Pow(2, 29);
StringBuilder m_sDisappearP = new StringBuilder(45546554);
StringBuilder m_sDisappearK = new StringBuilder(45546554);

bool m_bNoDelete = true;
int m_warnDeleteItems = 0; //do warning if more than how many items found disappeared, site settings.warn_if_how_many_items_disappeared

string m_sLastSupplierCode = "-1"; //dealing with bad csv file( multiple entries )
...
void Page_Load(Object Src, EventArgs E ) 
{
	...
	if(Request.QueryString["do_delete"] == "1")
		m_bNoDelete = false; //turn off protection
...
//*** for eden only DEBUG("root=", root); 
if(root.IndexOf("dealer\\") >= 0 )
	root = root.Replace("dealer\\", "");
...
//DEBUG(" rosd s=", root.IndexOf("dealer/").ToString());
		if(Directory.Exists(root))
		{
			...
			foreach(string sd in dirs)
			{
				...
				foreach (FileInfo f in di.GetFiles("*.csv")) 
				{
					...
					int rows = 0;
					string sc = "SELECT * FROM auto_update_log WHERE file_id= N'" + EncodeQuote(file_id) + "'";
					try
					{
					...
					Response.Write("<input type=button " + Session["button_style"] + " onclick=\"if(confirm('Are you sure want to Delete this file'))");
					Response.Write("{window.location=('autoupdate.aspx?t=delete&supplier=" + dir + "&file=" + HttpUtility.UrlEncode(file) + "');}\" value=Delete>");
					if(file.IndexOf('\'') >= 0)
					{
						Response.Write("<input type=button " + Session["button_style"] + " onclick=window.location=");
						Response.Write("\"autoupdate.aspx?t=delete&supplier=" + dir + "&file=" + HttpUtility.UrlEncode(file) + "\"; value=X>");
					}

					string sp = dir.ToUpper();
//					if(sp != "TP" && sp != "BB" && sp != "IM" && sp != "RN")// && sp != "CD")
//					if(sp != "TP" && sp != "IM" && sp != "RN")// && sp != "CD")
					if(sp != "TP" && sp != "RN")// && sp != "IM" && sp != "CD")
					{
						Response.Write("<input type=button " + Session["button_style"]);
						Response.Write(" onclick=window.location=(\"autoupdate.aspx?t=analyze&supplier=" + dir + "&file=" + HttpUtility.UrlEncode(file) + "\") value='Analyze'>");
					}
					Response.Write("</td>");
					Response.Write("</tr>");
				}
	...
	Response.Write("<font color=red> Disappeared: </font><font color=green>");
	...
	if(Directory.Exists(root))
	{
		...
		foreach(string sd in dirs)
		{
			...
			foreach (FileInfo f in di.GetFiles("*.csv")) 
			{
				...
				{
					string file_id = file + "_" + (f.Length/1000).ToString() + "K_" + f.LastWriteTime.ToString(date_format);
					string sc = " DELETE FROM auto_update_log WHERE file_id= N'" + EncodeQuote(file_id) + "'\r\n";
					sc += " INSERT INTO auto_update_log (file_id, updated_by) VALUES(N'" + EncodeQuote(file_id) + "', '" + Session["name"].ToString() + "')";
					try
					{
						myCommand = new SqlCommand(sc);
						myCommand.Connection = myConnection;
...
//			files_processed++;
//		}
		else if(String.Compare(target_supplier, "tp", true) == 0)
		{
...
//	return false;
	...
	if(skip)
	{
		...
		sc += " UPDATE code_relations SET supplier_price=" + dsupplier_price;
	...
	string rrp, string brand, string cat,  string s_cat, string ss_cat, ref Boolean skip, ref Boolean bIsNew)
{
	...
	DataRow dr;
//changes made to compare the correct supplie code.
	dracr = dsc.Tables["code_relations"].Select(" supplier_code = '"+ supplier_code +"'", "supplier_code ASC ");
	//dracr = dsc.Tables["code_relations"].Select("", "supplier_code ASC ");
//DEBUG("supds 	+", supplier_code );
//DEBUG("rrp price =", rrp);
	for(int i=0; i<dracr.Length; i++)
	{
		dr = dracr[i];
		code_index = i;
...
//DEBUG("supplier_code = "+ supplier_code, "csupserl d = "+ csupplier_code);
//DEBUG("nre ", nRet.ToString());
		if(nRet == 0)
{
			found = true;
			doCreateTempSupplierCodeTable(supplier, supplier_code, supplier + supplier_code);
}
...
//		DEBUG("ccc supse = ", csupplier_code);
	}
...
Boolean DumpCurrentCode(string supplier_code)
{
	...
	sc += " UPDATE code_relations SET disappeared = disappeared + 1 ";
	sc += " WHERE supplier = '" + target_supplier.ToUpper() + "' AND supplier_code = '" + supplier_code + "' ";
	if(g_bRetailVersion)
		sc += " END ";
//DEBUG("sc =", sc);
	try
...
//	}
	...
	string sc = "INSERT INTO code_relations_new (id, supplier, supplier_code, supplier_price, code, name, brand, cat, s_cat, ss_cat, skip, hot, rate, rrp)";
	sc += "VALUES('";
	sc += id;
	sc += "', '";
	sc += supplier;
...
//			DEBUG("Error, Duplite product code found ! supplier_code=" + supplier_code, " skip");
		...
		if(e.ToString().IndexOf("Invalid column name 'rrp'.") >= 0)
		{
			myCommand.Connection.Close();
			string scc = "ALTER TABLE code_relations_new Add rrp [money] ";
...
//DEBUG("rows="+dra.Length+" i=", i);
		...
		if(dra[i]["supplier_code"].ToString() == m_sLastSupplierCode)
			continue;
		m_sLastSupplierCode = dra[i]["supplier_code"].ToString();
		...
		bRet = ProcessDataRow(supplier, dra[i]["supplier_code"].ToString(), 
			...
			dra[i]["rrp"].ToString(), dra[i]["details"].ToString());
		if(!bRet)
			break;
		MonitorProcess(300);
		items++;
	}
/////////////delete all not found products from product table
//DEBUG(" m_sDisappearP +", m_sDisappearP.ToString());
//DEBUG(" m_sDisappeark +", m_sDisappearK.ToString());
	//if(m_sDisappearP.ToString() != "")
	{
		if(!DumpAllNotFoundProduct(supplier))
			return false;
	}


//DEBUG("found soup=", foundSupplier_code);
	int commas = 0;
	string sk = "";
	string sc = "";
//DEBUG("m_sDumpP=", m_sDumpP);
	if(m_sDumpP != "")
	{
		//prepare dumpping flags
		sc = " UPDATE product SET real_stock=0 ";
		sc += " UPDATE product_skip SET real_stock=0 ";
		sc += " UPDATE code_relations SET real_stock=0 ";
		sc += " UPDATE product SET real_stock = 1 WHERE code IN (SELECT code FROM stock_qty WHERE qty > 0) ";
		sc += " UPDATE product_skip SET real_stock = 1 ";
		sc += " WHERE id IN (SELECT c.id FROM code_relations c JOIN stock_qty q ON c.code = q.code WHERE q.qty > 0) ";
		sc += " UPDATE code_relations SET real_stock = 1 WHERE code IN (SELECT code FROM stock_qty WHERE qty > 0) ";
		try
		{
			myCommand = new SqlCommand(sc);
			myCommand.Connection = myConnection;
		...
		{
			int to_delete = 0;
			m_warnDeleteItems = MyIntParse(GetSiteSettings("warn_if_how_many_items_disappeared", "100"));
			if(m_sDumpP.Length <= 300)
			{
				sc = " SELECT code FROM product WHERE real_stock=0 AND supplier='" + target_supplier + "' ";
				sc += " AND supplier_code IN (" + m_sDumpP + ") ";
				try
				{
					myAdapter = new SqlDataAdapter(sc, myConnection);
					to_delete = myAdapter.Fill(dst, "no_delete");
				}
				catch(Exception e) 
				{
					ShowExp(sc, e);
			...
			else
			{
				commas = 0;
				sk = "";
				for(int j=0; j<m_sDumpP.Length; j++)
				{
					if(m_sDumpP[j] == ',')
					{
						commas++;
						if(commas >= 100 || j == m_sDumpP.Length - 1)
						{
							if(j == m_sDumpP.Length - 1)
							{
								sk += m_sDumpP[i]; //get the last character
							}
							sc = " SELECT code FROM product WHERE real_stock=0 AND supplier='" + target_supplier + "' ";
							sc += " AND supplier_code IN (" + sk + ") ";
							try
							{
								myAdapter = new SqlDataAdapter(sc, myConnection);
								to_delete += myAdapter.Fill(dst, "no_delete");
			...
			if(to_delete >= m_warnDeleteItems)
			{
				string d_url = "autoupdate.aspx?do_delete=1&" + Request.ServerVariables["QUERY_STRING"] + " class=o";
				Response.Write("<br><h4><font color=red>" + to_delete.ToString() + " items found disappeared, updating stopped, please check your file.</font></h4>");
				Response.Write("<h4>Click <a href=" + d_url + ">here</a> if you sure to delete all these " + to_delete.ToString() + " items</h4>");
				return false;
			}
		}
		...
		if(m_sDumpP.Length <= 300)
		{
			...
			sb.Append(m_sDumpP);
			sb.Append(") ");
			sb.Append(" UPDATE code_relations SET disappeared = disappeared + 1 ");
			sb.Append(" WHERE real_stock=0 AND supplier = '" + target_supplier + "' AND supplier_code IN (");
			sb.Append(m_sDumpP);
			sb.Append(") ");
			try
			{
			...
			for(int j=0; j<m_sDumpP.Length; j++)
			{
				if(m_sDumpP[j] == ',')
				{
					...
					if(commas >= 100 || j == m_sDumpP.Length - 1)
					{
						...
						sc = " DELETE FROM product ";
						sc += " WHERE supplier = '" + target_supplier + "' ";
						sc += " AND real_stock=0 AND supplier_code IN (";
						sc += sk;
						sc += ") ";
						sc += " UPDATE code_relations SET disappeared = disappeared + 1 ";
						sc += " WHERE real_stock=0 AND supplier = '" + target_supplier + "' ";
						sc += " AND real_stock=0 AND supplier_code IN (";
						sc += sk;
						sc += ") ";
						try
		...
		if(m_sDumpK.Length <= 300)
		{
			sc = " DELETE FROM product_skip WHERE real_stock=0 AND id IN (";
			sc += m_sDumpK;
			sc += ") ";
			try
			{
			...
			//update disappeared count
			sc = " UPDATE code_relations SET disappeared = disappeared + 1 ";
			sc += " WHERE real_stock=0 AND id IN (";
			sc += m_sDumpK;
			sc += ") ";
			try
			{
		...
		{
			Response.Write("<br>Deleteing disappeared items ... ");
			Response.Flush();
			
			commas = 0;
			sk = "";
			for(int j=0; j<m_sDumpK.Length; j++)
			{
				if(m_sDumpK[j] == ',')
				{
					commas++;
					if(commas >= 100 || j == m_sDumpK.Length - 1)
					{
						if(j == m_sDumpK.Length - 1)
						{
							sk += m_sDumpK[i]; //get the last character
						}

						sc = " DELETE FROM product_skip WHERE real_stock=0 AND id IN (";
						sc += sk;
						sc += ") ";
//since m_sDumpP == m_sDumpK, so no double increase disappeared counter DW
//						sc += " UPDATE code_relations SET disappeared = disappeared + 1 ";
//						sc += " WHERE real_stock=0 AND id IN (";
//						sc += sk;
//						sc += ") ";
...
{
	Response.Write("Cache product table......");
	Response.Flush();
...
Boolean CheckUpdates()
{
	...
	Response.Write("Checking updates<font color=green> ...");
	Response.Flush();
...
//DEBUG("code=" + drp["code"].ToString(), " price_dropped=" + drp["price_dropped"].ToString());
		...
		{
			if(AUNoRealStock(dr["code"].ToString()))
				bPriceChanged = true;
		}
...
bool AUNoRealStock(string code)
{
	if(dst.Tables["norealstock"] != null)
		dst.Tables["norealstock"].Clear();

	string sc = " SELECT qty FROM stock_qty WHERE code = " + code;
	try
	{
		myAdapter = new SqlDataAdapter(sc, myConnection);
		if(myAdapter.Fill(dst, "norealstock") > 0)
		{
			if(MyIntParse(dst.Tables["norealstock"].Rows[0]["qty"].ToString()) > 0)
				return false;
			else
				return true;
		}
...
//*** for eden only DEBUG("root=", root); 
if(strPath.IndexOf("dealer\\") >= 0 )
	strPath = strPath.Replace("dealer\\", "");
...
////delete all discountinued items...from product table
Boolean DumpAllNotFoundProduct(string supplier)
{
	string sc = " ";
//if(dst.Tables["deleteProduct"] != null)
//dst.Tables["deleteProduct"].Clear();
	sc = "SELECT TOP 1 count(ISNULL(code,0)) AS total_delete FROM product WHERE Upper(supplier) = '"+ supplier.ToUpper() +"'";
sc += " AND real_stock = 0 AND code NOT IN (SELECT s.code FROM stock_qty s WHERE s.code = product.code AND s.qty >0) AND supplier_code NOT IN (SELECT cf.supplier_code FROM cache_found_supplier_code cf WHERE ";
sc += " upper(cf.supplier) = '"+ supplier.ToUpper() +"' )";
//DEBUG("Sc =", sc);
	try
	{
		myAdapter = new SqlDataAdapter(sc, myConnection);
		myAdapter.Fill(dst, "deleteProduct");
		if(dst.Tables["deleteProduct"].Rows.Count > 0 )
		{
			for(int i=0;i<dst.Tables["deleteProduct"].Rows.Count; i++)
			{
				DataRow drw = dst.Tables["deleteProduct"].Rows[i];
				deleted_items = int.Parse(drw["total_delete"].ToString());		
		//	DEBUG("delte =", deleted_items);
			}
	...
	sc = " UPDATE code_relations SET disappeared = disappeared + 1 WHERE Upper(supplier) ='" + supplier.ToUpper() + "' ";
	//sc += " AND code NOT IN (SELECT s.code FROM stock_qty s WHERE s.code = code_relations.code) ";
	sc += " AND supplier_code NOT IN (SELECT supplier_code FROM cache_found_supplier_code WHERE upper(supplier) = '"+ supplier.ToUpper() +"' )";
	sc += " DELETE FROM product WHERE Upper(supplier) ='" + supplier.ToUpper() + "'";
	sc += " AND code NOT IN (SELECT s.code FROM stock_qty s WHERE s.code = product.code AND s.qty > 0)  ";
	sc += " AND real_stock = 0 AND supplier_code NOT IN (SELECT supplier_code FROM cache_found_supplier_code WHERE upper(supplier) = '"+ supplier.ToUpper() +"' )";
	sc += " DELETE FROM product_skip WHERE id in (SELECT product_skip FrOM cache_found_supplier_code WHERE upper(supplier) = '"+ supplier.ToUpper() +"' )";
	sc += " DELETE FROM cache_found_supplier_code WHERE Upper(supplier) = '" + supplier.ToUpper() +"' ";
//DEBUG("sc =", sc);
...
bool doCreateTempSupplierCodeTable(string supplier, string supplier_code, string product_skip)
{
	string sc = " INSERT INTO cache_found_supplier_code (supplier, supplier_code, product_skip) ";
	sc += " VALUES('"+ supplier +"', '" + supplier_code +"', '"+ product_skip +"' )";
//	sc += " WHERE supplier = '"+ supplier +"' ";
//DEBUG("sc =", sc);
	try
	{
	...
	{		
		if(e.ToString().IndexOf("Invalid object name 'cache_found_supplier_code'") >=0)
		{
		myCommand.Connection.Close();
			sc = @"

				CREATE TABLE [dbo].[cache_found_supplier_code] (			
			[supplier] [varchar](255) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL,
			[supplier_code] [varchar](255) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL,			
			[product_skip] [varchar](255)COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL	
			) ON [PRIMARY] 
				";
		}
		try
