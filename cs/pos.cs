...
//		string s_SearchMsg = "notfound";
	...
	if(Request.QueryString["t"] == "del")
	{
		...
		else
		{
			...
			else if(DoDeleteOrder())
			{
				...
				Response.Write("<br><br><center><h3>Order/Quote/Credit Notes Deleted.</h3>");
	...
	else if(Request.Form["cmd"] == "Update Order" || Request.Form["cmd"] == "Update Quote" || Request.Form["cmd"] == "Update Credit Notes")
	{
		PrintAdminHeader();
		PrintAdminMenu();
		if(DoUpdateOrder())
		{
			if(Request.Form["cmd"] == "Update Order")
				...
				Response.Write("<br><br><center><h3>Credit Notes Updated.</h3>");
			Response.Write("<input type=button  "+ Session["button_style"] +"  onclick=window.location=('");
	...
	else if(Request.Form["cmd"] == "Delete Order" || Request.Form["cmd"] == "Delete Quote" || Request.Form["cmd"] == "Delete Credit Notes")
	{
		PrintHeaderAndMenu();
...
//		Response.Write(" rmsg = window.prompt('Are you sure you want to delete this order?')\r\n");
		...
		else
		{
			...
			Response.Write("\\r\\nThis action cannot be undone.\\r\\n");
			Response.Write("\\r\\nClick OK to delete credit notes.\\r\\n");
		}
		Response.Write("'))");
...
//	sc += " ORDER BY oi.supplier_code DESC ";
	try
	{
		myAdapter = new SqlDataAdapter(sc, myConnection);
...
*/ 
			...
			Response.Write("else{window.history.go(-1)}");
			Response.Write("</script");
			Response.Write(">");
			return false;
		}
...
//	sc += " FROM product p JOIN code_relations c ON c.code = p.code ";
	...
	sc += " WHERE p.skip <>'1' AND ("; // supplier_code ";// LIKE '"+ kw;
...
//	DEBUG("tabwl =", tableWidth);
	...
	Response.Write("<td align=right>DISCOUNT(%)</td>");
...
bool PrintShipToTable()
{
    ...
    // add pic while order items
    int chkRows = dtCart.Rows.Count;
    Response.Write("<td>");
    if(chkRows >0)
    {
        int pic_row = chkRows-1;
        string pic_code = dtCart.Rows[pic_row]["code"].ToString();
        string picsrc = GetProductImgSrc(pic_code);
	    picsrc = picsrc.Replace("na.gif", "0.gif");
        Response.Write("<img src='" + picsrc +"' width=150 height=120 >");
    }
