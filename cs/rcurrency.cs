...
string m_type = "0";
string m_tableTitle = "Currency Loss Report";
string m_datePeriod = "";
string m_dateSql = "";
string m_code = "";
...
void PrintMainPage()
{
	PrintAdminHeader();
	PrintAdminMenu();
	Response.Write("<br><center><h4><b>Currency Loss Report</b></h4></center>");
	Response.Write("<form name=f action=rcurrency.aspx");
	if(m_salesID != "")
...
bool DoReport()
{
	...
	sc += " SELECT t.amount, d.trans_date, d.invoice_number, d.currency_loss AS loss ";
	sc += " FROM tran_detail d JOIN trans t ON t.id = d.id ";
	sc += " WHERE d.currency_loss <> 0 ";
	sc += m_dateSql;
//DEBUG("sc=", sc);
	try
	{
