...
Boolean DoSearch()
{
	...
	else
	{
        ...
        sc += " AND (LOWER(c.supplier_code) = N'" + EncodeQuote(m_sSupplier_code.ToLower()) + "' ";
        sc += " OR LOWER(b.barcode) = '" + EncodeQuote(m_sSupplier_code.ToLower()) + "' ";
        if(!TSIsDigit(m_sSupplier_code))
        {
            sc += " OR LOWER(c.name) LIKE N'%" + m_sSupplier_code.ToLower() + "%'";
        }
        else
            sc += " OR c.code = "+ EncodeQuote(m_sSupplier_code);
...
/**********Eric 11052012***********************/
	//string expire_date = EncodeQuote(Request.Form["expire_date"+sRow]);
/**********************************************/
...
//DEBUG("expire_date11=",expire_date);
/************************************/
...
//DEBUG("eaet =", eta);
	string sc = "";
//DEBUG("m_bSimpleInterface=",MyDoubleParse(m_bSimpleInterface.ToString()));
...
//return false;
	...
	else if(m_bFixedPrices)
	{
		...
		//string supplier = Rquest.Form["supplier"];
...
//check 3 packages barcode for existing barcodes
        ...
        double dCommissionRate = MyDoubleParse(Request.Form["commission_rate"]);
...
*/		
        sc += ", qpos_qty_break = " + qpos_qty_break;
        sc += ", commission_rate = " + dCommissionRate;
		sc += ", moq = '" + moq.ToString() + "' ";
...
//DEBUG("sc =",sc);		
		...
		if(expire_date != "")
			sb.Append(", expire_date=N'" + EncodeQuote(expire_date) + "' ");
		sb.Append(" WHERE id='");
		sb.Append(id);
		sb.Append("'");
		try
...
/****************************/
	if(sexpire != "")
		expire_date = DateTime.Parse(sexpire).ToString("dd/MM/yyyy");// DateTime.Parse(sexpire, format, System.Globalization.DateTimeStyles.NoCurrentDateDefault).ToString("dd-MM-yyyy");
...
//	Response.Write("<input type=hidden name=rate" + index + " value=" + dr["rate"].ToString() + ">");
	...
	Response.Write("<tr><td><font color=red><b>CODE/SID/SCODE</b></font></td><td>" + code + "/" + ssn + "/" + supplier_code);
...
//        	Response.Write("<option value='" + m_supplierID + "' selected><b>" + m_supplierName + "</b></option>");
            if(Request.QueryString["si"] == null)
            {
		        ...
		        Response.Write("<option value='" + m_supplierID + "' selected><b>" + showSupplierName(supplier) + "</b></option>");
            }
            else
		        Response.Write("<option value='" + m_supplierID + "' selected><b>" + m_supplierName + "</b></option>");
        }
        ...
        Response.Write("<option value= '" + supplier + "' >" + showSupplierName(supplier) +"</option>");
	Response.Write("</select>");
	Response.Write("</td></tr>");
    //DEBUG("m_supplierID", m_supplierID);
	//DEBUG("s=", supplier);
    if(supplier_code == "")
		supplier_code = id;
	Response.Write(" &nbsp;&nbsp; ");
	...
	Response.Write(name);
	Response.Write("'></td></tr>");
	Response.Write("<tr><td><B>"+Lang("Other Description") + "</b></td><td ><input type=text size=50  name=name_cn");
	Response.Write(index);
	Response.Write(" value='");
	...
	//BARCODE
	Response.Write("<tr><td><b>"+Lang("WEBPOS Barcode:")+"</b></td><td ><input type=text maxlength=20 size=30 name=barcode ");
 ...
 */ 
    ...
    Response.Write("<tr><td width=150><b><font color=green>"+Lang("Low Stock Warning")+" </b></td><td>");
	Response.Write("<input type=text size=5 name=low_stock value=" + low_stock + " style=text-align:right;>");
	//Response.Write(" (" +Lang("Individual Branch Stock Level")+")");
	Response.Write("</td></tr>");
...
//	Response.Write(" onclick=\"if(document.form1.barcode.value == ''){window.alert('Please enter barcode');return false;}\"");
	...
	Response.Write("<tr bgcolor='#EEEEE'><td colspan=2><font size=3><b>"+Lang("Cost Details")+"</b></font></td></tr>");
	Response.Write("<tr>");
/*********************************/
Response.Write("<td><table valign=center cellspacing=0 cellpadding=2 border=0 bordercolor=#EEEEEE bgcolor=white");
	Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:1px;border-style:Solid;border-collapse:collapse;fixed\">");
	...
	Response.Write("<tr><td width=150><b>"+Lang("Last Cost")+"(FRD)</b></td><td width=150>");
...
/////end the last cost table here 
	...
	Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:1px;border-style:Solid;border-collapse:collapse;fixed\">");
	Response.Write("<tr><td width=150><b>"+Lang("Last Cost")+""+"("+m_sCurrencyName+")"+"</b></td><td width=150>");
	...
	Response.Write("</tr></table>");
/********************************/
	Response.Write("<td valign=top width='"+ rowWidth +"'>");	
	Response.Write("<table align=left valign=center cellspacing=0 cellpadding=0 border=0 bordercolor=#EEEEEE bgcolor=white");
	Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:0px;border-style:none;border-collapse:collapse;fixed\">");
...
//	Response.Write("<tr><td><b>"+Lang("Manual Cost")+"("+Lang("FRD")+") <font color=red><i>*</i></font></b></td><td>");
	Response.Write("<input type=hidden size=5 style=text-align:right name=manual_cost_frd");
...
//	Response.Write("<tr><td><b>"+Lang("Manual Ex-Rate")+"</b></td><td>");
	Response.Write("<input type=hidden size=5 style=text-align:right name=manual_exrate");
...
//	Response.Write("<td><b>"+Lang("Manual COST")+""+"("+m_sCurrencyName+")"+"</b></td><td>");
	Response.Write("<input type=hidden size=5 style=text-align:right;background-color:#EEEEEE readonly=true name=manual_cost_nzd");
	Response.Write(" value=" + manual_cost_nzd + " "+ keyEnter +">");
//	Response.Write("</td></tr>");
//	Response.Write("</table>");
...
//	Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:0px;border-style:None;border-collapse:collapse;fixed\">");	
//	Response.Write("<tr>");
//	Response.Write("<td><b>"+Lang("Bottom Rate")+"</b></td><td>");
	Response.Write("<input type=hidden size=5 style=text-align:right name=rate" + index);
	Response.Write(" onchange=\"CalcPrice()\" value=" + rate + " "+ keyEnter +">");
//	Response.Write("</td></tr>");
//	Response.Write("<tr><td><b>"+Lang("Bottom Price")+"</b></td><td>");
	Response.Write("<input type=hidden size=5 style=text-align:right ");
	if(sClearance != "1")
		Response.Write(" readonly=true ");
	Response.Write("  name=price" + index + " value=" + price + " "+ keyEnter +">");	
//	Response.Write("</td></tr><tr><td><b>"+Lang("RRP")+"</b></td><td>");
	Response.Write("<input type=hidden size=5 style=text-align:right ");
	Response.Write(" name=rrp" + index + " value=" + MyDoubleParse(rrp).ToString() + " "+ keyEnter +">");
//	Response.Write("</td></tr>");
//	Response.Write("</table>");
	Response.Write("</td></tr>");
	Response.Write("</table>");
...
*/
	...
	{
		Response.Write("<tr bgcolor='#EEEEE'><td colspan='"+ cols +"'>");
		Response.Write("<b>"+Lang("RRP Price(exc GST)")+": </b><input size=5% style=text-align:right type=text name=wholeprice value='" + dealerLevel1Price +"'>");
		Response.Write("</tr>");
	}
	Response.Write("<tr><td colspan='"+ cols +"'>");
...
bool PrintFixedPricesInterface(DataRow dr, int row)
{
	...
	string supplier_price = dr["supplier_price"].ToString();
    double dCommissionRate = MyDoubleParse(dr["commission_rate"].ToString());
...
//		m_sBP += (" onkeyup=\"document.all.special_price.value = this.value;\"");
	...
	Response.Write("<font size=3><b>" + Lang("MULTI Barcodes") + "</b></font>");
	...
	*/
    ...
    Response.Write("<tr bgcolor='#EEEEE'><td width='"+ rowWidth +"' > ");
	Response.Write("<font size=3><b>" + Lang("Special Settings") + "</b></font>");
	//Response.Write(" &nbsp; <input type=button value='" + Lang("Edit Barcode") + "' onclick=window.location=('ebarcode.aspx?code=" + m_code +"') class=b></font></b>");
	Response.Write("</td>");
    Response.Write("<td><font size=3><b>" + Lang("MULTI Barcodes") + "</b></font>");
	Response.Write(" &nbsp; <input type=button value='" + Lang("Edit Barcode") + "' onclick=window.location=('ebarcode.aspx?code=" + m_code +"') class=b></font></b></td></tr>");
...
*/ 
    ...
    Response.Write("<tr bgcolor='#EEEEE'><td colspan=2><font size=3><b>" + Lang("Commission Settings") + "</b></font></td></tr>");	
	Response.Write("<tr><td>");
	...
	Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:1px;border-style:Solid;border-collapse:collapse;fixed\">");	
	Response.Write("<tr><td><b>" + Lang("Commission Rate") + "</b></td><td>");
	Response.Write("<input type=text size=10 name=commission_rate value='" + dCommissionRate + "'> ");
	Response.Write("%");
	Response.Write("</td></tr>");
	Response.Write("</table></td></tr>");
...
//POS Price
	...
	Response.Write("<font size=3><b>"+Lang("POS Prices")+"</b></font>");
	Response.Write("</td></tr>");	
	Response.Write("<tr><td>");
	Response.Write("<table align=left width='100%' valign=center cellspacing=0 cellpadding=2 border=0 bordercolor=#EEEEEE bgcolor=white");
	Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:1px;border-style:none;border-collapse:collapse;fixed\">");	
	Response.Write("<tr><td>");
	
	Response.Write("<table align=left width='100%' valign=center cellspacing=0 cellpadding=2 border=0 bordercolor=#EEEEEE bgcolor=white");
	Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:0px;border-style:none;border-collapse:collapse;fixed\">");	

	Response.Write("<tr><td wrap width='50%'>");
	Response.Write("<b>"+Lang("Sales Price (GST Inclusive)")+" </b>");
	Response.Write("</td><td>");
		
	Response.Write("<input size=10 name=price1 value=" + dPrice[0] + " style='");
	if(special != "0")
	...
	Response.Write(">");
	Response.Write("</td></tr><tr><td wrap><b>");
//	Response.Write(""+Lang("Discount Price (GST Inclusive)")+" </b>");
	Response.Write("</td><td><input type=hidden size=10 name=price2 value=" + dPrice[1] + " style=text-align:right; >");
	Response.Write("</td></tr><tr><td wrap>");
//	Response.Write("<b>"+Lang("Discount Qty")+" Qty(>=) </b>");
	Response.Write("</td><td><input type=hidden size=10 name=d_qty_break1 value=" + nQty[0] + " style=text-align:right; >");
	...
	Response.Write("<table align=left cellspacing=0 cellpadding=2 border=0 bordercolor=#EEEEEE bgcolor=white");
	Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:0px;border-style:none;border-collapse:collapse;fixed\">");	
...
//	Response.Write("<b>"+Lang("Gross Profit")+" ($)</b>");
	Response.Write("</td><td><input type=hidden style='text-align:right;background-color:#EEEEEE' size=5 readonly name=grossprofit value='"+ grProfit.ToString("c") +"' >");
	Response.Write("</td></tr><tr><td align=right>");
//	Response.Write("<b>(%)");
	Response.Write("</td><td><input type=hidden size=5 readonly style='text-align:right;background-color:#EEEEEE' name=grPercent value='"+ dGPPercent.ToString("p") +"' >");
	Response.Write("</td></tr>");
	Response.Write("</table>");			
	Response.Write("</td></tr>");
	Response.Write("</table>");
