...
{	
	...
	string sCheckLine = "Code,SupplierCode,Barcode,Stock";
	if(line != sCheckLine)
	{
		Response.Write("<h4>Error, invalid file, first line must be:</h4><font color=red>" + sCheckLine + "</font><br>");
		return;
	}
	line = r.ReadLine();
	string text = "";
...
//			DoUpdateInStock(code, supplier_code, barcode, dQty);
		if(!DoUpdateInStock(code, supplier_code, barcode, dQty))
			break;
		line = r.ReadLine();
	}
	r.Close();
...
bool DoUpdateInStock(string code, string supplier_code, string barcode, double dQty)
{
	string sc = "";
	if(code == "")
	{
		...
		if(barcode != "")
		{
			sc = " SELECT code FROM code_relations WHERE barcode = '" + EncodeQuote(barcode) + "' ";
		}
		else if(supplier_code != "")
		{
			sc = " SELECT code FROM code_relations WHERE supplier_code = '" + EncodeQuote(supplier_code) + "' ";
		}
		try
		{
			...
			if(myAdapter.Fill(ds, "get_code") <= 0)
            {
	            ...
	            sc += " branch_id, qty, barcode, new_item) ";
	            sc += " VALUES ( ";
