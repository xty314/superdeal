...
//			return;
		...
		{
			Response.Write("<meta http-equiv=\"refresh\" content=\"0; URL=import_a.aspx?t=analyze&file=" + HttpUtility.UrlEncode(m_file) + "\">");
		}
		return;
	}
	...
	{
		string root = "data/updateitem";
		root = Server.MapPath(root);
		string pathname = root + "\\" +  m_file;
		if(File.Exists(pathname))
			File.Delete(pathname);
		Response.Write("<meta http-equiv=\"refresh\" content=\"0; URL=import_a.aspx\">");
		return;
	}
	...
	else
	{
		...
		foreach (FileInfo f in di.GetFiles("*.csv")) 
		{
			...
			Response.Write("<tr><td><a href=import_a.aspx?supplier=" + dir + "&file=");
			...
			Response.Write("<td>");
			Response.Write("<input type=button " + Session["button_style"] + " onclick=window.location=('import_a.aspx?t=process&file=" + HttpUtility.UrlEncode(file) + "') value=Process>");
			Response.Write("<input type=button " + Session["button_style"] + " onclick=window.location=('import_a.aspx?t=delete&file=" + HttpUtility.UrlEncode(file) + "') value=Delete>");
			string sp = dir.ToUpper();
			Response.Write("<input type=button " + Session["button_style"] + " onclick=window.location=('import_a.aspx?t=analyze&file=" + HttpUtility.UrlEncode(file) + "') value='Analyze'>");
			Response.Write("</td>");
			Response.Write("</tr>");
		}
...
bool PrintAnalyzePage()
{
	...
	Response.Write("<form action=import_a.aspx?file=" + HttpUtility.UrlEncode(m_file) + " method=post>");

	for(int i=0; i<aNameCount; i++)
	{
		string sName = aName[i].ToUpper();
		if(sName == "PRICE1" || sName == "M_PN" || sName == "BARCODE" || sName == "DESCRIPTION" 
		|| sName == "MOQ" || sName == "OUTER_PACK" || sName == "CAT" || sName == "S_CAT"
		|| aName[i] == "lines_to_skip")
        {
            if(sName == "PRICE1")
			    sName = "PRICE";
            else if(sName == "M_PN")
 ...
 */
	...
	Response.Write("<input type=button " + Session["button_style"] + " onclick=window.location=('import_a.aspx?r=" + DateTime.Now.ToOADate() + "') value=OK>");
	Response.Write("</td></tr>");
	Response.Write("</form>");
	Response.Write("</table>");
...
//DEBUG("rpie =", price1);
    ...
    if(ItemExists(supplier_code, ref existedCode))
	{
//DEBUG("	existedCode =", existedCode);
        sc += " BEGIN ";
		sc += " UPDATE product SET name = N'" + EncodeQuote(name) + "' WHERE code = "+ existedCode +" ";
        sc += " UPDATE code_relations SET name = N'" + EncodeQuote(name) + "', level_price0 = "+ price1 +" , barcode = '"+ EncodeQuote(barcode)+"' ";
        sc += ", cat = N'" + EncodeQuote(cat) + "' ";
        sc += ", s_cat = N'" + EncodeQuote(s_cat) + "' ";
        sc += ", ss_cat = N'" + EncodeQuote(ss_cat) + "' ";
		sc += " WHERE code = "+ existedCode + " ";
        sc += " UPDATE stock_qty SET qty = 10000  WHERE code = "+ existedCode;
        sc += " END ";
        sc += " DELETE FROM barcode WHERE barcode = '" + EncodeQuote(barcode) +"' ";
        sc += " INSERT INTO barcode (item_code, barcode, item_qty, carton_qty, carton_barcode, box_qty, package_price, supplier_code) ";
		sc += " VALUES ('"+ existedCode +"', '"+ EncodeQuote(barcode)+"',1,0,'',1,'0','"+ EncodeQuote(supplier_code)+"')";
	}
    else
	{
		...
		sc += " INSERT INTO stock_qty (code, qty, supplier_price, average_cost) VALUES(" + nCode.ToString() + ", 10000, " + supplier_price + ", " + supplier_price + ") ";
		sc += " ELSE UPDATE stock_qty SET qty = 10000 WHERE code = "+ nCode.ToString() +" ";
...
bool ItemExists(string id, ref string code)
{
	...
	string sc = " SELECT code FROM code_relations WHERE supplier_code = '" + EncodeQuote(id) + "' ";
//DEBUG("sc =", sc);
	try
	{
		myAdapter = new SqlDataAdapter(sc, myConnection);
...
bool DoProcessBarcodeSupplier()
{
	PrintAdminHeader();
	Response.Write("Opening file....");
	string root = "data/updateitem/barcodeandsupply.csv";
	root = Server.MapPath(root);
	string fileName = root + m_file;
//DEBUG("fileName = ", fileName);
	FileStream fs = new FileStream(fileName, FileMode.Open, FileAccess.Read);
...
bool DoProcessProduct()
{
	PrintAdminHeader();
	Response.Write("Opening file....");
	string root = "data/updateitem/product.csv";
	root = Server.MapPath(root);
	string fileName = root + m_file;
//DEBUG("fileName = ", fileName);
	FileStream fs = new FileStream(fileName, FileMode.Open, FileAccess.Read);
...
bool DoProcessMOQ()
{
	PrintAdminHeader();
	Response.Write("Opening file....");
	string root = "data/updateitem/moq.csv";
	root = Server.MapPath(root);
	string fileName = root + m_file;
//DEBUG("fileName = ", fileName);
	FileStream fs = new FileStream(fileName, FileMode.Open, FileAccess.Read);
...
bool DoProcessBook1()
{
	PrintAdminHeader();
	Response.Write("Opening file....");
	string root = "data/updateitem/book1.csv";
	root = Server.MapPath(root);
	string fileName = root + m_file;
//DEBUG("fileName = ", fileName);
	FileStream fs = new FileStream(fileName, FileMode.Open, FileAccess.Read);
