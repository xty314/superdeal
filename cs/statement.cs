...
//		sTimeLbl = "<font color=blue><b>&nbsp;&nbsp; To Be Billed/Emailed </b><i>( >=30 days)</i></font>";
	...
	Response.Write("&nbsp;&nbsp;&nbsp;<span id=printable-copy-s><input id=printable-copy type=button " + Session["button_style"] + " onclick=window.open('"+ s_vdUrl + "&t=vd') value=' Printable Copy '></span>");
	...
	Response.Write("<td width=15% align=left><input type=checkbox name=selected_all_invoice><b>Invoice No.</b></td>");
	...
	for(int i = 0; i < dst.Tables["invoice_rec"].Rows.Count; i++)
	{
		...
		Response.Write("<tr class=statement-list>");
		...
		if(!bCardType(ref m_custID))
			Response.Write("<input type=checkbox name=selected_invoice value=" + dr["invoice_number"].ToString() + "><a href=invoice.aspx?n=" + dr["invoice_number"].ToString() + "");
		else
			Response.Write("<a href=purchase.aspx?t=pp&n=" + dr["pid"].ToString() + "");
