...
{
	//if(m_sSite == "www")
    Response.Write("<br><center><h2>Order Trace");
	if(m_sSite == "admin")
	{
		if(ds.Tables["order"].Rows.Count > 0)
			m_customerName = ds.Tables["order"].Rows[0]["trading_name"].ToString();
		Response.Write(" - <font color=#fa7c63>" + m_customerName + "</font>");
	}
	else
	{
		Response.Write(" - <font color=#fa7c63>");
		if(m_bSystem)
    ...
    if(m_sSite == "admin")
        ...
        Response.Write("<td align=left style='border:none;font-size:15px'>");
	...
	Response.Write("<i class=\"fa fa-chevron-circle-right\" style='color: #fa7c63;'></i> <a href=status.aspx?t=1"); if(Request.QueryString["ci"] != null)Response.Write("&ci=" + Request.QueryString["ci"]);Response.Write("&r=" + DateTime.Now.ToOADate() + " class=o>Being Processed</a>&nbsp&nbsp&nbsp&nbsp;");
	Response.Write("<i class=\"fa fa-chevron-circle-right\" style='color: #fa7c63;'></i> <a href=status.aspx?t=2"); if(Request.QueryString["ci"] != null)Response.Write("&ci=" + Request.QueryString["ci"]);Response.Write("&r=" + DateTime.Now.ToOADate() + " class=o>Invoiced</a>&nbsp&nbsp&nbsp&nbsp;");
	Response.Write("<i class=\"fa fa-chevron-circle-right\" style='color: #fa7c63;'></i> <a href=status.aspx?t=3"); if(Request.QueryString["ci"] != null)Response.Write("&ci=" + Request.QueryString["ci"]);Response.Write("&r=" + DateTime.Now.ToOADate() + " class=o>Shipped</a>&nbsp&nbsp&nbsp&nbsp;");
	Response.Write("<i class=\"fa fa-chevron-circle-right\" style='color: #fa7c63;'></i> <a href=status.aspx?t=4"); if(Request.QueryString["ci"] != null)Response.Write("&ci=" + Request.QueryString["ci"]);Response.Write("&r=" + DateTime.Now.ToOADate() + " class=o>Back Ordered</a>&nbsp&nbsp&nbsp&nbsp;");
	Response.Write("<i class=\"fa fa-chevron-circle-right\" style='color: #fa7c63;'></i> <a href=status.aspx?t=5"); if(Request.QueryString["ci"] != null)Response.Write("&ci=" + Request.QueryString["ci"]);Response.Write("&r=" + DateTime.Now.ToOADate() + " class=o>OnHold</a>&nbsp&nbsp&nbsp&nbsp;");
	Response.Write("<i class=\"fa fa-chevron-circle-right\" style='color: #fa7c63;'></i> <a href=status.aspx?r=" + DateTime.Now.ToOADate()); if(Request.QueryString["ci"] != null)Response.Write("&ci=" + Request.QueryString["ci"]);Response.Write(" class=o>All</a> ");
	Response.Write("</td>");
	Response.Write("<td align=right style='border:none;'><form action=status.aspx method=get>");
	Response.Write("<div class=\"form-inline\"><input type=text class='form-control account-input' size=25 placeholder='Search for order ID' name=kw autocomplete=off value=" + Session["status_order_search_kw"] + ">");
	Response.Write("<input type=submit class='btn btn-info' style='margin-left:10px;background-color: #fa7c63;border-color: #fa7c63;' " + Session["button_style"] + " name=cmd value='Search'>");
	Response.Write("</div></form></td>");
	Response.Write("</tr>");

	Response.Write("<tr><td colspan=2 style='border:none;'>");
	...
	Response.Write("<tr style=\"color:white;background-color:#fa7c63;font-weight:bold;\">");
...
//	Response.Write("<br><center><h3>Order Information</h3>");
	...
	Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:1px;border-style:Solid;border-collapse:collapse;fixed\">");
	Response.Write("<tr style=\"color:white;background-color:#2e5ea0;font-weight:bold;border-color:#e1e1e1;\">");
...
*/
	...
	for(int i=0; i < rows; i++)
	{
		...
		Response.Write("<tr style=\"border-color:#e1e1e1;\"");
		if(bKit)
