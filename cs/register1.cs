...
protected void Page_Load(Object Src, EventArgs E ) 
{
	...
	if(!TS_UserLoggedIn() && Session["terms_read"] == null)
	{
	   string rsout = ReadSitePage("public_left_side_menu");
		...
		Response.Write(" <br/><br/><input type=submit style=\"background:url(pic/continue.gif); width:100px; height:20px;border:none;\" " + Session["button_style"].ToString() + " name=cmd value=''>");
		Response.Write("</form></center></td></tr></table></center>");
	}
	else
	{
		if(TS_UserLoggedIn())
...
//	else if(dr["fax"].ToString() == "")
//		err = "Error, Fax Number can't be blank.";
	else if(dr["name"].ToString() == "")
		err = "Error, Purchse Manger can't be blank.";
/*	else if(Request.Form["agree_terms"] != "on")
		err = "Error, you must agree terms of sales.";
	else if(Request.Form["disclose"] != "on")
		err = "Error, you must agree to not disclose your login details to anyone.";
	*/
	if(err != "")
...
//		password = FormsAuthentication.HashPasswordForStoringInConfigFile(password, "md5");
	string type = GetEnumID("card_type", "dealer");//Request.Form["type"];
...
//	string country = Request.Form["country"];
	...
	bool bRet = AddNewDealer(email, password, trading_name, corp_number, directory, gst_rate, 
		...
		sm_ddi, sm_mobile, ap_name, ap_email, ap_ddi, ap_mobile, "", GetEnumID("card_type", "customer").ToString(), "");
...
//		url += "/"+ m_sCompanyName;
		...
		msgMail.Subject = "Application Form Received";
		msgMail.BodyFormat = MailFormat.Html;
		msgMail.Body += "<br>\r\nCompany : " + company + "<br>\r\n";
		msgMail.Body += "PM Manager : " + name + "<br>\r\n";
		msgMail.Body += "PM EMail : " + email + "<br><br>\r\n";
...
//	if(Session["main_card_id"].ToString() == "") //manager loggedin
	...
	for(int i=0; i<rows; i++)
	{
		...
		if(bEdit && Request.QueryString["id"] == id)
		{
			...
			Response.Write("<td><input type=submit name=cmd value=Update " + Session["button_style"] + ">");
