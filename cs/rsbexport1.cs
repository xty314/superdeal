...
bool DoPaymentSummary()
{
    ...
    sc += " SELECT s.id, s.invoice_number, s.code, s.quantity, s.name, s.supplier_code, s.commit_price, s.discount_percent, CONVERT(VARCHAR(10), i.commit_date, 105) AS invoice_date, i.cust_ponumber AS job_num ";
	sc += " FROM sales s JOIN ";
...
//return true;
    ...
    string job_num = "";
    if(ds.Tables["report"].Rows[0]["job_num"] != null && ds.Tables["report"].Rows[0]["job_num"] != "")
    ...
    if(Request.Form["cmd"] == "Export Invoice")
	{
        ...
        bRet = WriteCSVFile(ds, "invoice_" + m_invoice_num + "-job_" + job_num);
