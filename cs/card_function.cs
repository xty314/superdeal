<!-- #include file="statement_cust.cs" -->
<script runat=server>
...
bool AddNewDealer(string email, string pass, string trading_name, string corp_number, string directory, string gst_rate, 
				  ...
				  string ap_name, string ap_email, string ap_ddi, string ap_mobile, string short_name, string type, string barcode, string our_branch)
{
	if(IsDuplicateEmail(email))
	{
		Response.Write("<center><h3>Error, this email address has already been used</h3>");
	...
	sc += " name, pm_email, pm_ddi, pm_mobile, sm_name, sm_email, sm_ddi, sm_mobile, ap_name, ap_email, ap_ddi, ap_mobile, our_branch) ";
	sc += " VALUES('" + email + "', ";
	if(pass != "")
		sc += "'" + pass + "', ";
	sc += type + ", ";
	...
	sc += ap_name + "', '" + ap_email + "', '" + ap_ddi + "', '" + ap_mobile + "', '" + our_branch + "' )";
...
bool IsDuplicateBarcode(string barcode)
{
	DataSet dsa = new DataSet();
	int rows = 0;
...
//	Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:1px;border-style:Solid;border-collapse:collapse;fixed\">");
		...
		|| (m_sSite == "admin" && !bNew))
	{
		...
		if(m_sSite != "www")
		{
			Response.Write("<td><table width=100%><tr><td align=left style=\"font:bold 14px arial; color:#000000\">"+sType+ " ID: " +id+"</td><td align=right><b>CARD TYPE : </b><select name=type>");	
...
//	if(newType != "employee" && sType !="EMPLOYEE" && newType != "customer" && sType !="CUSTOMER")
...
//		Response.Write(" <a href=branch.aspx class=o>Branch List</a>");
	...
	//if(newType != "employee" && sType !="EMPLOYEE" && !bIsBranch )//&& newType != "customer" && sType !="CUSTOMER")
	...
	if(newType != "customer" && sType !="CUSTOMER")
	{
		...
		Response.Write("<tr><td bgcolor=#eeeeee align=center>");
		Response.Write("ID Number</td><td><input class='form-control account-input' type=text size=55 name=corp_number maxlength=50 value='" + corp_number + "'></td></tr>");
	}
...
//DEBUG("sType",sType);
	...
	Response.Write("</font><span class='required'>*</span></td><td><input id=delivery_address_1 class='form-control account-input' type=text size=20 name=address1 maxlength=30  onkeyDown=\"this.style.backgroundColor='#FFFFFF'\" value='");
...
//		Response.Write("<input type=hidden name=currency_for_purchase value=1>");
		if(newType !="employee" && sType !="EMPLOYEE" && newType !="customer" && sType !="CUSTOMER" && newType != "member" && sType !="MEMBER" && newType !="otherse" && sType !="OTHERS" && m_sSite == "admin" )
		{
			Response.Write("<tr><td bgcolor=#eeeeee align=center>");
			...
			if (newType != "customer" && sType !="CUSTOMER" && newType != "member" && sType !="MEMBER")	    
			{
                ...
                if(!TS_UserLoggedIn())
			    {
				    ...
				    //Response.Write("<tr><td colspan=2><input type=checkbox name=disclose> ");
				   	//Response.Write("I am over 18 years old.</td></tr>");
			    }
			    else
			    {
			...
			//stop placing order
		        ...
		        else //www site register
		        {
			        ...
			        if(!TS_UserLoggedIn())
			        {
				        ...
				        Response.Write("<tr><td colspan=2><input type=checkbox name=disclose> ");
				        Response.Write("I agree that I will not disclose my login details to anyone.");
        Response.Write(" <div class='g-recaptcha' data-sitekey='6LdOstcUAAAAAMXdaEvI53kByXGrOrNKssaeaH9e'></div></td></tr>");
			        }
			        else
			        {
				        Response.Write("<input type=hidden name=agree_terms value=on> ");
	...
	else
	{
		if(TS_UserLoggedIn())
		{
			if(SecurityCheck("manager", false))
			{
				...
				{
					if(!bApproved)
						Response.Write("<input type=submit class=b name=cmd value='Approve'>&nbsp&nbsp;");
					else
						Response.Write("<input type=submit name=cmd class=b value='Send Password'>&nbsp&nbsp;");
					
					if(Session["last_uri_exp"] != null && Session["last_uri_exp"] != "")
						Response.Write("<input type=button class=b name=cmd value='<< Back to Expense' Onclick=\"window.location=('"+ Session["last_uri_exp"] +"')\">");

					if(id != "0")
					{
					Response.Write("<input type=checkbox name=delete> Delete this card ");
					Response.Write("<input type=submit class=b name=cmd value='Delete'>&nbsp;&nbsp;");
					}
				}
			}
			if(m_sSite != "admin")
			{
				Response.Write("<input class=\"btn btn-warning\" type=button class=b onclick=window.location=('setpwd.aspx') value='Change Password'>");
			}
		}
		if(Request.QueryString["v"] == "view")
			...
			Response.Write("<input type=submit class=b name=cmd value='Update'></td></tr>");
	}
...
bool PrintDealerLevelOptions(string current_level)
{
	...
	if(dst.Tables["getdealerlevel"] != null)
		dst.Tables["getdealerlevel"].Clear();
	
	int levels = 0 ;//MyIntParse(GetSiteSettings("dealer_levels", "3"));
	string sc = "";
	sc = " SELECT * from enum WHERE 1=1 and class = 'dealer_level_name' ORDER by id";
	try
	{
		myAdapter = new SqlDataAdapter(sc, myConnection);
		levels = myAdapter.Fill(dst, "getdealerlevel");
	}
	catch(Exception e) 
	{
		ShowExp(sc, e);
...
//		levels = 9;
	...
	{
		DataRow dr = dst.Tables["getdealerlevel"].Rows[i];
