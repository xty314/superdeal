...
/*
    sc = " SET DATEFORMAT dmy ";
    sc += " SELECT '' AS tid, r.id AS recon_id, r.record_date AS recon_date, r.close_balance AS traN_amount, r.trans_ref AS ref, 'reconcile' AS recon_type ";
    sc += " FROM acc_recon r ";
    sc += " WHERE r.acc_number = "+ m_account +"";
DEBUG("last acc recon = ", sc );
	try
	{
		myAdapter = new SqlDataAdapter(sc, myConnection);
...
//DEBUG("sc=", sc);
	...
	Response.Write("<td colspan=4 align=right><font color=green><b>Bank Balance</b></font></td>");
	Response.Write("<td align=right>");
...
//		string date = DateTime.Parse(dr["trans_date"].ToString()).ToString("dd-MM-yyyy");
	...
	if(tran_type == "purchase")
        ...
        dClosingBalance = (tran_type == "purchase" || tran_type == "expense")? dClosingBalance -= dAmount: dClosingBalance += dAmount;
        //if(tran_type == "purchase" || tran_type == "expense")
...
//		Response.Write("<td>" + payment + "</td>");
		...
		else
		{
			if(bMarked)
				...
				Response.Write("<td align=right><a href="+  Request.ServerVariables["URL"] +"?acc=" + m_account + "&row=" + i + "&ssid=" + m_ssid + " class=o title='Mark Amount'>" + dAmount.ToString("c") + "</a></td>");
		}
...
//	Response.Write("<td>&nbsp;</td>");
	...
	Response.Write("<tr><td colspan=4 align=right nowrap>");
...
//DEBUG("ticket = ", ticked);
            ...
            case "expense":
                ...
                break;
            case "purchase":
                sc += " UPDATE trans SET reconcile = 1 WHERE id = "+ tran_id +" ANN banked = 0 ";
                break;
            case "deposit":
...
void PrintJavaCode()
{
	...
	sb.Append("{						\r\n");
	sb.Append("		var opening_balance = 0; //Number(document.f.opening_balance.value);		\r\n");
