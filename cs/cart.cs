		...
		else if(Request.QueryString["c"] != null)
		{
			...
			if(TSIsDigit(code))
			{
				...
				else
				{
					string qty = "0";
					//DEBUG("code=",code);
					//DEBUG("qty=",Request.QueryString["qty"].ToString());
	...
	for(int i=0; i<dtCart.Rows.Count; i++)
	{
		...
		if(dr["code"].ToString() == code && dr["site"].ToString() == m_sCompanyName)
		{
            ...
            if(m_sSite != "admin" && g("t") == "b")
			{
			   ...
			   Response.Write("already exists, update quantity"); 
			}
			return true;
		}
	}
	...
	ref double dRowGST, ref double dRowTotal, ref double dRowSaving)
{
	...
	sb.Append("<td align=center valign=middle nowrap  width='150px' height='60px'>");
	sb.Append("<img style=\"width:300px;\" src=" + src + " border=\"0\">");
	// sb.Append("<img  src=" + src + " border=\"0\">");
	sb.Append("</td>\r\n");
