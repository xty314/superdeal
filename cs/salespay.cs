...
string m_status = "";
bool m_bRefund = false;
bool m_bStatusRefunded = false;
...
void Page_Load(Object Src, EventArgs E ) 
{
	...
	if(Request.QueryString["refund"] == "1" || Request.Form["refund"] == "1")
		m_bRefund = true;
...
//DEBUG("inv=", m_invoiceNumber);
	...
	if(m_invoiceNumber == "")
	{
		Response.Write("<meta http-equiv=\"refresh\" content=\"0; URL=order.aspx?j=refund&r=" + DateTime.Now.ToOADate() + "\">");	
		return;
	}
	...
	{
		Response.Write("<meta  http-equiv=\"refresh\" content=\"0; URL=close.htm\">");
		return;	
	}
	...
	if(Request.QueryString["acc"] != null)
	{
		string accname = "income_account";
		if(m_salesType == "online")
			accname = "income_account_for_online_sales";
		if(!SetSiteSettings(accname, m_account))
			return;
	}
...
bool RecordTransaction()
{
	...
	{
		Response.Write("<h3>Error, No Amount Entered.</h3>");
		return false;
	}

	string payment_method = Request.Form["payment_method"];
	string sTotal = Request.Form["total"];
	double dTotal = double.Parse(sTotal, NumberStyles.Currency, null);
...
//	myCommand.Parameters.Add("@amount_for_card_balance", SqlDbType.Money).Value = sTotal;
	...
	myCommand.Parameters.Add("@Amount", SqlDbType.Money).Value = dTotal;
	...
	if(m_bRefund)
	{
		...
		sc = "UPDATE invoice SET paid=0, type=" + receipt_type + ", refunded=1 ";
	}
	else
	{
		receipt_type = GetEnumID("receipt_type", "invoice"); //fully paid
		status_close = GetEnumID("general_status", "closed");
		sc = "UPDATE invoice SET paid=1, payment_type=" + payment_method + ", type=" + receipt_type;
		sc += ", status=" + status_close;
	}
	...
	if(m_bRefund)
		sc += GetEnumID("sales_item_status", "order placed");
	else
	...
	if(m_bDodiscount)
	{
		if(m_bRefund)
			dTotal = 0 - dTotal;
		if(!UpdateDiscount(Request.Form["card_id"], dTotal.ToString()))
			return false;
	}
	return true;
}
...
//DEBUG("rows=", rows);
	...
	if(rows == 1)
	{
		...
		m_bStatusRefunded = bool.Parse(dst.Tables["inv"].Rows[0]["refunded"].ToString());
	}
	return true;
}
...
//DEBUG("rows=", rows);
	...
	Response.Write("('salespay.aspx?i=" + m_invoiceNumber);
	if(m_bRefund)
		Response.Write("&refund=1");
	Response.Write("&acc='+this.options[this.selectedIndex].value)\">");
...
//	double dTotal = dAmount + dTax;
	...
	if(m_bRefund)
		...
		Response.Write("Sales Payment");
	Response.Write("</h3></center>");
	Response.Write("<form name=form1 action=salespay.aspx?i=" + m_invoiceNumber);
	if(m_bRefund)
		Response.Write("&refund=1");
	Response.Write(" method=post>");
	if(m_bRefund)
		Response.Write("<input type=hidden name=refund value=1>");
	...
	Response.Write("\r\n<input type=text name=total onclick=\"if(this.value=='')this.value=" + dTotal + ";\" ");
	if(m_bPayAll)
...
//	Response.Write("<tr><td colspan=2>&nbsp;</td></tr>");
	...
	Response.Write("<tr><td>");
	if(m_status != "closed" || m_bRefund)
	{
		Response.Write("<button onclick=window.location=('salespay.aspx?t=payall&i=" + m_invoiceNumber);
		if(m_bRefund)
		{
			Response.Write("&refund=1");
			Response.Write("')>Refund All</button>");
		}
		else
			Response.Write("')>Pay All</button>");
	}
	if(m_salesType == "online")
		Response.Write("<button onclick=window.location=('esales.aspx?i=" + m_invoiceNumber + "')>Delete</button>");
	...
	{
		if(m_salesType == "online")
			Response.Write("<button onclick=window.location=('esales.aspx?i=" + m_invoiceNumber + "')>Shipping</button>");
	}
	else
	{
		if(!m_bStatusRefunded)
