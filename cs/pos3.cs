...
//DEBUG("ADMIN ",  Session["employee_access_level"].ToString());
	...
	m_dGstRate = MyDoubleParse(GetSiteSettings("gst_rate_percent", "12.5")) / 100;						//26.06.2003 NEO	
...
bool RestoreOrder()
{
	...
	int items = 0;
	string sc = " SELECT oi.discount_percent, oi.*, o.sales ";
...
//	sc += " FROM product p JOIN code_relations c ON c.code = p.code Where 1=1 "; // LIKE '"+ kw;
	...
	else
	{
		...
		else
		{
			...
			{
				sc += " AND (p.code = (SELECT TOP 1 b.item_code FROM barcode b WHERE b.barcode ='"+ kw +"') OR p.code ='"+ kw +"' OR LOWER(p.supplier_code) ='"+kw+"')";
			}
			else
				sc += " AND (Lower(p.supplier_code) = '"+ kw +"' OR p.code ='"+ kw +"')";
		}
	}
//DEBUG(" sc =",sc);
//return false;
...
//	Response.Write("<tr bgcolor=#ffffff><td colspan=" + (m_cols-1).ToString() + " align=right>");
	Response.Write("<td bgcolor=#EEEEE colspan=3 align=right>");
...
//	sc += " AND main_card_id IS NULL  ";
	...
	{
		sc += " AND (name LIKE '%" + kw + "%' OR email LIKE '%" + kw + "%' OR trading_name LIKE '%" + kw + "%' OR company LIKE '%" + kw + "%') ";
...
//return false;
	try
	{
		...
		if(myCommand1.Fill(dsco, "id") == 1)
		{
			...
			sc += ", customer_gst = (SELECT ISNULL(gst_rate,'0.125') FROM card WHERE id ='"+ card_id +"' ) ";
