...
Boolean DoSearch()
{
    ...
    if(Request.Form["search"] != null && Request.Form["search"] != "")
        sb.Append(" TOP 1 ");
    sb.Append(" c.id, c.name_cn, c.price1, c.level_price0, c.code, c.brand, c.name, c.cat, c.s_cat, c.ss_cat, c.average_cost,c.supplier_code, c.supplier_price, c.barcode ");
...
//	sb.Append(" LEFT OUTER JOIN specials s on c.code=s.code ");
	if(Request.Form["search"] != null && Request.Form["search"] != "")
	    sb.Append(" LEFT OUTER JOIN barcode b ON c.code = b.item_code");
	string swhere = "";
	if(m_bService)
		swhere += " AND c.is_service = 1 ";
	else
	{
		if(brand != null)
			swhere += " AND c.brand= N'" + brand + "'";
		else if(cat != null && cat != "")
			swhere += "  AND c.cat= N'" + cat + "'";

		if(s_cat != null && s_cat != "")
		{
			if(swhere != "")
				swhere += " AND ";
			swhere += " c.s_cat= N'" + s_cat + "'";
		}
		if(ss_cat != null && ss_cat != "")
		{
			if(swhere != "")
				swhere += " AND ";
			swhere += " c.ss_cat= N'" + ss_cat + "'";
		}
		if(m_supplier != null)
		{
			if(swhere != "")
				swhere += " AND ";
			swhere += " c.supplier= N'" + m_supplier + "'";
		}
		if(Request.Form["search"] != null && Request.Form["search"] != "")
			//swhere += " AND b.barcode ='"+ Request.Form["search"] +"'";
            swhere += " AND ";
	}
	if(swhere != "")
		sb.Append(" WHERE 1=1 " + swhere);
...
if(Request.Form["search"] != null && Request.Form["search"] != "")
{
	...
	if(TSIsDigit(Request.Form["search"].ToString()))
		sb.Append(" (c.code = "+ kw +" OR b.barcode = '"+ kw +"' OR c.supplier_code =N'"+ kw +"') ");
	else
	{
		kw = EncodeQuote(kw);
		sb.Append(" (b.barcode = '"+ kw +"' OR c.supplier_code = N'"+ kw +"' OR LOWER(c.name) LIKE N'%"+ kw.ToLower() +"%' OR LOWER(c.name_cn) LIKE N'%"+ kw.ToLower() +"%' ) ");
    ...
    {
        if(Request.QueryString["order"] == "scode")
            sb.Append(" ORDER BY c.supplier_code ");
    }
	else
        sb.Append(" ORDER BY c.supplier_code, c.brand, c.s_cat, c.ss_cat, c.name, c.code");
...
*/
	...
	Response.Write("<td height='30' class='pageName' id='top-header2' ><font size=3><font size=+1><b>"+ tableTitle +" (<span style='color:red;'>"+ m_iTotalItems.ToString()+"</span>) </b><font color=red><b>");
...
//	sb.Append(" style=\"font-family:Verdana;font-size:8pt;border-width:1px;border-style:Solid;border-collapse:collapse;fixed\">");
	...
	sb.Append("<td width='10%'>DELETE</td>");
	sb.Append("<td>EDIT</td>");
	sb.Append("<td width=50><a href=# onclick=\"window.location=('eprice.aspx?order=scode')\" >CODE</a></td>");
	sb.Append("<td>BRAND</td>");
	sb.Append("<td>NAME (DESCRIPTION)</td>");
    sb.Append("<td>Other NAME</td>");	
...
Boolean DrawRow(DataRow dr, int i, Boolean alterColor)
{
	...
	string code = dr["supplier_code"].ToString();
    ...
    double price = double.Parse(dr["level_price0"].ToString());
...
*/
	...
	if(bHaveStock)
		...
		sb.Append("<a href=dp.aspx?" + s_code + " class=o>DEL</a> ");
	sb.Append("</td>");
	sb.Append("<td><a href=liveedit.aspx?code=" + s_code + " class=o>EDIT</a> </td>");
	sb.Append("<td>" + code + "</td>");
	sb.Append("<td>" + brand + "</td>");
	sb.Append("<td>" + name + "</td>");
    sb.Append("<td>" + otherName + "</td>");
    sb.Append("<td align=center>" + price.ToString("c") + "</td>");
...
*/
	...
	sb.Append(""+ GetAllBranchStock(s_code) +"");
...
bool PrintCatalogSelection()
{
	...
	if(m_supplier != null)
		sc += " WHERE supplier=N'" + m_supplier + "' ";
	sc += " ORDER BY cat";
	try
	{
...
//	Response.Write("<form action=eprice.aspx?d=" + m_supplier + "&r=" + DateTime.Now.ToOADate() + " method=Get>");
	Response.Write("&nbsp; <b>Select Catalog :</b> <select name=c");
...
bool CreatePDFFile()
{
	...
	string sFN = "temp/catalog_" + DateTime.Now.ToString("yy_MM_dd") + ".pdf";
	string sPath = sFN;
...
string PrintBrochureLayout()
{
	...
	if(rows > 0)
	{
		sBody = sBody.Replace("@@CAT", dst.Tables["product"].Rows[0]["cat"].ToString());
		sBody = sBody.Replace("@@SCAT", dst.Tables["product"].Rows[0]["s_cat"].ToString());
		readHeader = readHeader.Replace("@@CAT", dst.Tables["product"].Rows[0]["cat"].ToString());
