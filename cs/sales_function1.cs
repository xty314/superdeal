...
*/
	...
	m_dGstRate = MyDoubleParse(dr["gst_rate"].ToString());
...
{
	if(Request.Form["discount_total"] == null || Request.Form["discount_total"] == "")
		return true;
	bool bRate = false;
	
	string sdt = Request.Form["discount_total"];
//	sdt = "%"+sdt;
	bool bIncludeGST = MyBooleanParse(GetSiteSettings("discount_include_GST", "0", true));
	bIncludeGST = false;	
	double dDiscountRate = 0;
	
	if(sdt.IndexOf("%") == 0)
	{
		//DEBUG("ROUNT ", sdt.IndexOf("%").ToString());
		//return false;
		bRate = true;
		int nLength = sdt.Length;
		if(nLength <= 1)
			return true;
		sdt = sdt.Substring(1, nLength - 1);
		//DEBUG("ROUNT ", sdt);
		//return false;
	}
	try
	{
		double dd = double.Parse(sdt);
		if(dd == 0)
			return true;
	}
	catch(Exception e)
	...
	//GST inclusive 
	double dGSTRate = dCustomerGSTRate;
	//DEBUG("dGSTRate", dGSTRate);

	if(bIncludeGST)
	{
		dGSTRate = dCustomerGSTRate;
		sdt = (double.Parse(sdt) / ( 1 + (dGSTRate))).ToString();
		//DEBUG("dGSTRate", dGSTRate);
...
//DEBUG("sdt=", sdt);
	...
	double dNewTotal = MyMoneyParse(sdt);
	double dTotal = MyMoneyParse(Request.Form["subtotal"]);
//DEBUG("dNewTotal", dNewTotal);

	if(bManualSetGST)
		dTotal = dTotal / (1 + (dGSTRate));
//DEBUG("dTotal", dTotal);	
//	double dDiscount = dTotal - dNewTotal;
	double dRate = dNewTotal/dTotal;
//DEBUG("rate=", dRate);	
	double dDiscountTotal = 0;
	double dFreight = MyMoneyParse(Request.Form["freight"]);
	dDiscountTotal += dFreight;

	if(bRate) //calculate by percent
	{
		dRate = 1 - MyDoubleParse(sdt) / 100;
		
	//	DEBUG("dtotal ", dTotal.ToString());
	//	return false;
		dNewTotal = dTotal * dRate * (1 + m_dGstRate);
...
//DEBUG("Price in cart =", dPrice.ToString());
		//dPrice =  dPrice* (1 + m_dGstRate);
//	DEBUG("Price in cart =", dPrice.ToString());
//DEBUG("rate=", dRate);
		double dNewPrice = dPrice * dRate;
//DEBUG("Price in cart =", dNewPrice.ToString());		
		double dQty = MyDoubleParse(dtCart.Rows[i]["quantity"].ToString());
		bool bKit = MyBooleanParse(dtCart.Rows[i]["kit"].ToString());
//DEBUG("<br>rows=", i);
		if(i == 0) //the last one
		{
//			double dLeft = (dNewTotal - dDiscountTotal) / (1 + m_dGstRate);
			double dLeft = dNewTotal - dDiscountTotal;
			dNewPrice = dLeft / dQty;
		//	DEBUG("dNewPrice", dNewTotal.ToString());

		}
		else if(bCheckBottomPrice && !bKit)
		{
			string code = dtCart.Rows[i]["code"].ToString();
			DataRow drp = null;
			...
			double dBottomPrice = dManualCostNZD;
			if(dLastCostNZD < dManualCostNZD)
				dBottomPrice = dLastCostNZD;
			
			if(dNewPrice <= dBottomPrice)
			{
				Response.Write("<h3>Bottom Price Reached</h3>");
				dNewPrice = dBottomPrice;
			}
		}

//		dDiscountTotal += dNewPrice * dQty * (1 + m_dGstRate);
		dDiscountTotal += dNewPrice * dQty;
		dtCart.Rows[i].BeginEdit();
			//////calculate gst price again...
		dNewPrice = dNewPrice / (1 + dGSTRate); 	
	
		if(sdt != "" && dDiscountRate == 0)
			dDiscountRate = MyDoubleParse(sdt) / 100;
											
       	//DEBUG("dNewPrice", dDiscountRate.ToString());
		//return false;
		dtCart.Rows[i]["salesPrice"] = dNewPrice ;
		dtCart.Rows[i]["discount_percent"] = (dDiscountRate * 100);
