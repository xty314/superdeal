...
//	Response.Write(Request.Form["brand"]);
	Response.Write("</td></tr>");
	Response.Write("<tr><td>Supplier Code<font color=red><b>*</b></font></td><td><input type=text name=supplier_code size=30 maxlength=100 value='");
...
/**************************
	string s = "\r\n<select name=supplier";
	s += " onchange=\"window.location=('addp.aspx?";
	s += "sup='+ escape(this.options[this.selectedIndex].value))";
	s +="\">";
/**************************/
...
string GetSupplierId(string shortname)
{
	int rows = 0;
	if(ds.Tables["supplierID"] != null)
		ds.Tables["supplierID"].Clear();
	string sc = "";
	sc += " SELECT top 1 id from card where short_name like '"+shortname+"'";
	try
	{
		myAdapter = new SqlDataAdapter(sc, myConnection);
		rows = myAdapter.Fill(ds, "supplierID");
		if(rows <= 0)
			return "";
	}
	catch(Exception e) 
	...
	DataRow dr = ds.Tables["supplierID"].Rows[0];
	string supplier_id = dr["id"].ToString();
	return supplier_id;
}
...
bool DoSave()
{
	...
	Boolean bRet = true;
	string supplier = EncodeQuote(Request.Form["supplier"]);// GetSupplierId(Request.Form["supplier"]);
