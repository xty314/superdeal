...
void Page_Load(Object Src, EventArgs E ) 
{
	...
	if(Request.Form["cmd"] == Lang("Settlement"))
//	if(Request.QueryString["settle"] == "1")
	{
		if(DoSettle())
		{
...
Boolean InsertToSettle(string sRow)
{
	Boolean bRet = true;
...
//	{
		...
		sc += " IF not EXISTS(SELECT * FROM stock_settle WHERE code=" + code + " AND branch_id = " + branch + ") ";
		sc += " INSERT INTO stock_settle (staff, code, stock, branch_id) VALUES(" + Session["card_id"].ToString();
		sc += ", " + code + ", " + qty_old + ", " + branch + ") "; 
		
		sc += " ELSE ";
		sc += " UPDATE stock_settle SET stock =" + qty_old + ", settle_time=getdate() WHERE code=" + code + " AND branch_id = " + branch;
		//sc += " INSERT INTO stock_settle (code, stock, branch_id) VALUES(" + code + ", " + qty_new + ", " + branch + ") ";
//	}
	sc += " SELECT IDENT_CURRENT('stock_settle') AS id ";
	sc += " COMMIT ";
	if(sc == "")
		return true;
	...
	{
		SqlDataAdapter myCommand = new SqlDataAdapter(sc, myConnection);
		if(myCommand.Fill(dsid, "id1") != 1)
		{
			Response.Write("<br><center><h3>Error getting new record ID");
			return false;
		}
		new_id = dsid.Tables["id1"].Rows[0]["id"].ToString();
	}
	catch(Exception e) 
	{
		ShowExp(sc, e);
...
bool DrawRow(DataRow dr, int i, bool alterColor)
{
	...
	Response.Write(" </td><td><a href=stockinouttrace.aspx?p=0&c=" + code + " class=o>" + Lang("Trace") + "</a> ");

    Response.Write("</a>&nbsp;&nbsp;<a href=stocktrace.aspx?p=0&c=" + code + " class=o>" + Lang("Current") + "</a> ");    
...
/*	if(m_bBarcode)
	...
	*/
	...
	Response.Write("<td align=center><input type=text name=stock_location"+i+" value='" + itemLocation(code) + "' ></td>");	
	Response.Write("</tr>");
	return true;
}
...
void PrintPageIndex()
{
    ...
    Response.Write("<input type=submit " + Session["button_style"] + " name=cmd value='" + Lang("Settlement") + "' >");

	Response.Write("</td>");
	Response.Write("</tr>");

	Response.Write("<tr><td align=right colspan=7><font color=red>");
	Response.Write("Before Settlement, please Update Adjustment first!! &nbsp;&nbsp;</font>");
	Response.Write("</td></tr>");
}
