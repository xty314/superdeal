...
void Page_Load(Object Src, EventArgs E ) 
{
	...
	if(Request.QueryString["pid"] != null && Request.QueryString["pid"] !="")
	   Session["expense_payee"] = Request.QueryString["pid"];
	...
	{
		EmptyExpenseTable();
		Response.Write("<meta http-equiv=\"refresh\" content=\"0; URL=expense.aspx?sid=");
	...
	else if(Request.QueryString["t"] == "del")
	{
		...
		Response.Write("<meta http-equiv=\"refresh\" content=\"0; URL=expense.aspx?r="+ DateTime.Now.ToOADate() +"");
		if(Request.QueryString["sid"] != null && Request.QueryString["sid"] != "")
...
//		m_bIsPaid = MyBooleanParse(Request.QueryString["pd"].ToString());
	...
	else if(Request.QueryString["saydone"] == "1")
	{
		...
		if(m_id != "")
		{
			...
			Response.Write("<br><br><a title='go to expense report' href='explist.aspx?s="+ Session["card_id"] +"&id="+ Request.QueryString["acid"] +"&type=1&sid="+Request.QueryString["sid"]+"' class=o>Go to Expense Report</a>");
			Response.Write("<br><br><a title='go to expense records' href='"+ Request.ServerVariables["URL"] +"?id="+ m_id +"&sid="+Request.QueryString["sid"]+"' class=o>Back to Expense Record</a>");
			return;
//			Response.Write(", please wait 1 second...<meta http-equiv=\"refresh\" content=\"2; URL=expense.aspx?id=" + m_id + "\">");
	...
	else if(Request.Form["cmd"] == "Record")
	{
		...
		if(DoRecordExpense())
		{
			Response.Write("<script language=javascript>window.location=('expense.aspx?saydone=1&id=" + m_id +"&acid="+ Request.Form["to_account"] +"&sid="+ Request.QueryString["sid"]+"');</script");
			Response.Write(">");
		}
	...
	else if(Request.Form["cmd"] == "Update Record")
	{
		...
		{
			//Response.Redirect("expense.aspx?saydone=1&id=" + m_id);
			  Response.Write("<meta http-equiv=\"refresh\" content=\"0; URL="+ Request.ServerVariables["URL"] +"?r="+ DateTime.Now.ToOADate() +"&id=" + m_id +"&sid="+ Request.QueryString["sid"]+"");
			  Response.Write("&pid="+Request.Form["supplier"]+""); 
			  Response.Write("\">");
			  return;
		}
	}
...
//	sb.Append("</td></tr>");
	...
	if(Session["expense_customer"] != null && Session["expense_customer"] != "")
		  Response.Write("?sid="+Request.QueryString["sid"]+"");
		  Response.Write("&pid="+ Session["expense_customer"] +"");
	Response.Write("' method=post>");

	//hidden values
	Response.Write("<input type=hidden name=id value=" + m_id + ">");
//DEBUG("expense_customer",Session["expense_customer"].ToString());
	...
	Response.Write("<tr><td valign=top>");
		...
		if(Session["expense_customer"] != null && Session["expense_customer"] != "" && Session["expense_payee"] =="")
			Response.Write(PrintSupplierOptions(Session["expense_customer"].ToString(), "", "others"));
		else if(Session["expense_payee"] != null && Session["expense_payee"] != "")
			Response.Write(PrintSupplierOptions(Session["expense_payee"].ToString(), "", "others"));
		else
...
/*		Response.Write("<input type=hidden name=customer value=" + m_customerID + ">");
		...
		*/
		...
		if(!PrintToAccountType())
			return false;
	    string checkClassId = Request.QueryString["sid"];
	    if(checkClassId !=""){
...
//		Response.Write("<td>&nbsp;</td>");
	...
	if(m_bRecorded)
	{
		...
		Response.Write(">");
		Response.Write("<input type=button value='Expense List' onclick=window.location=('explist.aspx?sid="+Request.QueryString["sid"]+"') " + Session["button_style"] + ">");
	}
	else
	{
...
bool PrintToAccountType()
{
	...
	string getClassId = Request.QueryString["sid"];
	Response.Write("<select name=to_account_type onchange=\"window.location=('" + Request.ServerVariables["URL"] + "?r=" + DateTime.Now.ToOADate());
	Response.Write(" &sid='+this.options[this.selectedIndex].value)\">");
	Response.Write ("<option>-- Please Select --</option>");
	for(int i=0; i<rows; i++)
	{
		DataRow dr = dst.Tables["toaccounttype"].Rows[i];
		//string id = dr["id"].ToString();
		//string number = dr["class1"].ToString() + dr["class2"].ToString() + dr["class3"].ToString() + dr["class4"].ToString();
		//string disnumber = dr["class1"].ToString() + "-" + dr["class2"].ToString() + dr["class3"].ToString() + dr["class4"].ToString();
		string classNo = dr["class1"].ToString();
		//double dAccBalance = double.Parse(dr["balance"].ToString());
		Response.Write("<option value="+classNo );
	 if(classNo == getClassId)
	 {
			Response.Write(" selected");
			//m_sAccBalance = dAccBalance.ToString("c");
	 }
...
{
	int rows = 0;
	string getClassSid = Request.QueryString["sid"];
	//string sc = "SELECT *, name4+' ' +name1 AS type "; // Take out accunt type name
	string sc = "SELECT *, name4 AS type ";
	sc += " FROM account ";
	//sc += " WHERE class1 = 6 OR class1 = 2 ";
	if(getClassSid  !="")
	sc += "WHERE class1="+ getClassSid;
...
bool DoCustomerSearchAndList()
{
	...
	Response.Write(" onClick=window.location=('expense.aspx') " + Session["button_style"] + ">");
...
m_customerID = Request.Form["supplier"];
	...
	sc += " INSERT INTO expense (branch, card_id, from_account, to_account, payment_type ";
...
bool DoUpdateRecord()
{
	...
	//record items
	sc = " DELETE FROM expense_item WHERE id = " + m_id;
	sc += " SET DATEFORMAT dmy ";
	double dSubTax = 0;
	double dSubTotal = 0;
	for(int i=0; i<dtExpense.Rows.Count; i++)
