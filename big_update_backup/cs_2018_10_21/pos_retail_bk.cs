...
void Page_Load(Object Src, EventArgs E ) 
{
	...
	m_dGstRate = MyDoubleParse(GetSiteSettings("gst_rate_percent", "12.5")) / 100;
...
void BindISTable()
{
	...
	for(int i=0; i<m_nSearchReturn; i++)
	{
		...
		Response.Write("&supplier_code=" + supplier_code + "&pri=" + price + "&r=" + DateTime.Now.ToOADate() + "'>");
...
//	if(m_quoteType != "3" || Request.QueryString["p"] == "new")
	Response.Write("<tr><td colspan=6><input type=text name=item_code_search size=8 value=''>");
...
//	Response.Write("<tr><td colspan=3><input type=submit name=cmd  "+ Session["button_style"] +"  size=8 value='Select From Categories'></td></tr>");
...
//	Response.Write("<tr bgcolor=#EEEEE><td colspan=" + m_cols + " align=right>");
...
*/	
	...
	Response.Write("<b>Discount Total(GST Exclusive):</b></td><td align=right><input type=text name=discount_total size=5 style=text-align:right>");
	...
	Response.Write(" this.value=Math.round("+  dAmount +" - document.all.eftpos.value - document.all.cheque.value - document.all.cc.value,2); this.select();\" ");
	...
	Response.Write("this.value=Math.round("+  dAmount +" - document.all.cheque.value - document.all.cash.value-document.all.cc.value,2);this.select();\" ");
	Response.Write(" onKeyUp=\" if(cal('payment') == false){ this.value='';}\" ");
	Response.Write(" style=\"text-align:right\">");
	Response.Write("</td></tr>");
	...
	Response.Write("this.value=Math.round("+  dAmount +" - document.all.eftpos.value - document.all.cash.value - document.all.cheque.value,2);this.select();\" ");
	Response.Write(" onKeyUp = \"if(cal('payment')== false){this.value='';}\" ");
	Response.Write(" style=\"text-align:right\">");
	Response.Write("</td></tr>");
	...
	Response.Write("this.value=Math.round("+  dAmount +" - document.all.eftpos.value - document.all.cash.value - document.all.cc.value,2);this.select();\" ");
	Response.Write(" onKeyUp = \"if(cal('payment')== false){this.value='';}\" ");
	Response.Write(" style=\"text-align:right\">");
	Response.Write("</td></tr>");
...
//DEBUG(" level =", dst.Tables["card"].Rows[0]["dealer_level"].ToString());
	...
	Response.Write("<input type=button onclick=window.location=('pos.aspx?ci=0&r=" + DateTime.Now.ToOADate() + "') value='Cash Sales'  "+ Session["button_style"] +" >");
...
bool WriteOrderItems(string order_id)
{
	...
	for(int i=0; i<dtCart.Rows.Count; i++)
	{
		...
		sc += ", " + Math.Round(MyDoubleParse(dr["salesPrice"].ToString()), 3) + ") ";
