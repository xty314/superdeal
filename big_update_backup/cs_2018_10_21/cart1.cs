...
bool AlreadyExists(string code)
{
	for(int i=0; i<dtCart.Rows.Count; i++)
	{
		...
		if(dr["code"].ToString() == code && dr["site"].ToString() == m_sCompanyName)
		{
			...
			int nNewQty = MyIntParse(Request.QueryString["qty"]);
			nQty += nNewQty;
			...
			Response.Write("{window.location='?t=b&c=" + code + "&qty=" + nNewQty.ToString() + "&confirm=0'};");
...
//		Session["purchase_exrate"] = GetSiteSettings("exchange_rate_" + GetEnumValue("currency", drp["currency"].ToString()));
	...
	else
	{
		...
		else
		{
			...
			else if(Session[m_sCompanyName + "loggedin"] != null)
			{
				if(int.Parse(GetSiteSettings("dealer_levels", "1")) > 0 && Session["card_type"].ToString() != "1") //set dealer to get dealer level price
					dr["salesPrice"] = dBottomPrice * MyDoubleParse(drp["level_rate"+ level +""].ToString());
				else
...
//DEBUG("sf=", sShippingFee);
	...
	{
		sb.Append("<input type=button value='Continue Shopping' " + Session["button_style"].ToString() + " OnClick=window.location=('" + Session["item_list_url"] + "');>");
		...
		if(m_bOrder)
			...
			sb.Append("') value='Continue Checkout'>");//Continue to checkout</button>");
	}
	sb.Append("\r\n</td></tr></table>\r\n</td></tr>");
...
//DEBUG("amount=", dAmount.ToString("C"));
	...
	sb.Append("window.location=('cart.aspx?reset=1')></td>");
