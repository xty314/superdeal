...
bool GetData()
{
	...
	if(m_sSupplier != "")
	{
		sc = " SELECT c.supplier, c.name, c.name_cn, c.code, c.low_stock, c.supplier_code, s.qty, c.allocated_stock ";
	...
	{
		sc = " SELECT top 30 c.supplier, c.name, c.name_cn, c.code, c.low_stock, c.supplier_code, s.qty, c.allocated_stock ";
		sc += " FROM code_relations c JOIN stock_qty s ON c.code = s.code ";
		sc += " WHERE 1=1 ";
        if(m_sLow == "1")
        {
