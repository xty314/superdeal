...
//	}
	...
	//item search
	if((Request.Form["item_code_search"] != null && Request.Form["item_code_search"] != "") || (Request.Form["item_option"] != null && Request.Form["item_option"] != ""))
	{
...
//		string s_SearchMsg = "notfound";
		...
		{	
			if(IsInteger(Request.Form["item_code_search"]) || IsInteger(Request.Form["item_option"]) )
			{
                ...
                if(Request.Form["item_option"] != null && Request.Form["item_option"] != "")
                {
...
//	if(m_quoteType != "3" || Request.QueryString["p"] == "new")
    ...
    if(!g_bPDA)
        ...
        Response.Write("<input type=submit name=cmd  "+ Session["button_style"] +"  size=8 value='Add Item'>");
...
bool PrintItemSeletion()
{
	...
	string sc = "SELECT code, name FROM code_relations WHERE 1=1 ";
    sc += " ORDER BY name";
	try
	{
		SqlDataAdapter myCommand = new SqlDataAdapter(sc, myConnection);
	...
	Response.Write("<select name=item_option>");
