...
//	if(s_cat != null && ss_cat == null && m_sSite == "admin")
	...
	if(m_bClearance)
	{
		sc = "SELECT c.barcode, p.price_age, p.supplier, p.supplier_code, p.supplier_price, p.code, p.name, p.brand, p.price";
		if(bIncludeGST)
...
//DEBUG("sc cleanrl =", sc);
	...
	{
		sc = "SELECT c.barcode, c.code, c.name, c.brand, p.price";
		if(g_bRetailVersion)
		{
			sc += ", ISNULL((select sum(qty";
			if(bEnable_Allocated_Stock)
...
//DEBUG("sc speical =", sc);
	...
	{
		sc = " c.barcode, p.price_age, p.supplier, p.supplier_code, p.supplier_price, p.code, p.name, p.brand, p.price";
		if(bIncludeGST)
			sc += "* "+ (1+Double.Parse(Session[m_sCompanyName +"gst_rate"].ToString()))  +" AS price";
		if(g_bRetailVersion)
		{
		...
		if(scWhere != "")
		{
			...
			if(cross > 0)
			{
				for(int c=0; c<cross; c++)
				{
					...
					sc = " SELECT id as barcode, '01/01/2001' AS price_age, '' AS supplier, '' AS supplier_code, 0 AS supplier_price ";
...
//DEBUG("mbshowlogo =", m_bShowLogo.ToString());
	...
	if(bOrder)
	{
		...
		//set dealer level price over write fix price setting in sales mode
//		if(MyBooleanParse(GetSiteSettings("enable_dealer_level_price_in_sales_mode", "0", true)) && m_nDealerLevel > 1 )
//			m_bFixedPrices = false;
	}
	...
	string printablefooter = ReadSitePage("public_body_item_print_footer");
	if(g_bOrderOnlyVersion)
		tp = ReadSitePage("public_body_item_list_orderonly");
	if(!bShowSpecial){
	    tp = ReadSitePage("public_body_item_list_print");
		tp = tp.Replace("@@printablefooter", printablefooter);
		}
	...
	if(rows > m_nPageSize)
	{
		...
		if(m_nPage > 1)
		{
			slPagePrev = WriteURLWithoutPageNumber() + "&kw="+ HttpUtility.UrlEncode(keywordSearch) +"&p=" + (m_nPage-1).ToString();
			slPageNext = WriteURLWithoutPageNumber() + "&kw="+ HttpUtility.UrlEncode(keywordSearch) +"&p=" + (m_nPage).ToString();
			slPageLast = WriteURLWithoutPageNumber() + "&kw="+ HttpUtility.UrlEncode(keywordSearch) +"&p=" + pages.ToString();
			slPageFirst = WriteURLWithoutPageNumber() + "&kw="+ HttpUtility.UrlEncode(keywordSearch) +"&p=1";
		}
		else
		{
			slPagePrev = WriteURLWithoutPageNumber() + "&kw="+ HttpUtility.UrlEncode(keywordSearch) +"&p=1";			
			slPageFirst = WriteURLWithoutPageNumber() + "&kw="+ HttpUtility.UrlEncode(keywordSearch) +"&p=1";			
		}
		for(int p=1; p<=pages; p++)
		{
			...
			{
				slPage += "<a href=";
				slPage += WriteURLWithoutPageNumber() + "&kw="+ HttpUtility.UrlEncode(keywordSearch) +"&p=" + p.ToString();
				slPage += "> ";
			}
			else
		...
		if(m_nPage < pages)
		{
			slPageNext = WriteURLWithoutPageNumber() + "&kw="+ HttpUtility.UrlEncode(keywordSearch) +"&p=" + (m_nPage+1).ToString();
			slPageLast = WriteURLWithoutPageNumber() + "&kw="+ HttpUtility.UrlEncode(keywordSearch) +"&p=" + pages.ToString();
		}
...
//		tp = tp.Replace("@@LEFT_SIDE_MENU", lsm);
	...
	if(tp.IndexOf("@@SUB_SUB_MENU") >= 0)
		tp = tp.Replace("@@SUB_SUB_MENU", "");
	else
		tp = tp.Replace("@@LEFT_SIDE_MENU", "");
...
//					title_desc = " - All Others";
      ...
      string FirstCat = "";
	  string BrandName = Request.QueryString["b"];
	      if(BrandName =="")
		     FirstCat = Request.QueryString["s"];
			 else
			 FirstCat = BrandName;
	...
	sout = sout.Replace("@@First_Cat", FirstCat);
...
//DEBUG("cad =", card_id);
			...
			{				
				/*DataRow drr = GetCardData(card_id);
				if(drr == null)
					m_bFixedPrices = true;
				else
				{
					string sCardType = drr["type"].ToString();
					string sCardName = drr["name"].ToString().ToLower();
			
					if(sCardType == "4" && !bOrder && m_sSite.ToLower() == "admin")
						m_bFixedPrices = true;							
					else if(sCardType == "0" || sCardType == "1" || (sCardName).ToLower().IndexOf("cash sales") >= 0)
						m_bFixedPrices = true;				
					else
						m_bFixedPrices = false;
				}
...
//					DEBUG("tysep =", sCardType);
				...
				else
				{
					if(Session[m_sCompanyName + "loggedin"] != null)
					{
						...
						m_bFixedPrices = true;
//				DEBUG(" sdf m_bFixedPrices =", m_bFixedPrices.ToString());
				}
...
//DEBUG(" m_bSimpleInterface = ", m_bSimpleInterface.ToString());
//DEBUG(" m_bFixedPrices +", m_bFixedPrices.ToString());
//DEBUG(" m_bUseLastSalesFixedPrice =", m_bUseLastSalesFixedPrice.ToString());
//DEBUG("dprice =", dPrice);
	if(m_bFixedPrices)
		nLevel = 1;
//DEBUG("nleve =", nLevel);
...
{	double dTmpPrice = GetLastSalesFixedPriceForDealer(code, "1", card_id, nLevel.ToString());
	if(dTmpPrice > 0)
		dPrice = dTmpPrice;
	//dPrice = GetLastSalesFixedPriceForDealer(code, "1", card_id, nLevel.ToString());
}
//DEBUG("rpic e=",dPrice.ToString());
...
///DEBUG("m_bNoPrice", m_bNoPrice.ToString());
//DEBUG("m_bDiscontinued", m_bDiscontinued.ToString());
...
//DEBUG(" m_dGSTRate+", m_dGSTRate);
	...
	if(bHasGroup && sbRows.Length > 0) //row behinde
	{
		...
		if(bFirstGroup)
		{
			...
			sgroup = sgroup.Replace("@@ITEM_LIST_SORT_LINK", slSort);
			sgroup = sgroup.Replace("@@ITEM_LIST_SORT_NAME", sSortName);
			sgroup = sgroup.Replace("@@BRAND", dr["brand"].ToString());
			sgroup = sgroup.Replace("@@Printable",  Request.ServerVariables["QUERY_STRING"]);
			bFirstGroup = false;
		}
