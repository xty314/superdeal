...
//DEBUG("m_inv_sNumber=",m_inv_sNumber);
//DEBUG("sInvoiceNumber=", sInvoiceNumber);	
...
string BuildItemTable(DataTable dt, bool bOrder, string sHeader, string sFooter)
{
	...
	sb.Append("<td width=1%>Line No.</td>");
...
//		sb.Append("<td width=6%><b>STO </b></td>\r\n");
	...
	{	
		...
		if(MyBooleanParse(dt.Rows[i]["kit"].ToString()))
		{
			...
			sb.Append(InvoicePrintOneRow(i, dt.Rows[i], bSystem, bIndividual_Price, true, bOrder));
		}
		else
		{
			sb.Append(InvoicePrintOneRow(i, dt.Rows[i], bSystem, bIndividual_Price, bOrder));
		}
		j++;
...
//		if(bOrder && bool.Parse(dt.Rows[0]["credit_paid"].ToString()))
		if(bOrder)
		{
			if(bool.Parse(dt.Rows[0]["credit_paid"].ToString()))
			{
...
string InvoicePrintOneRow(int index, DataRow dr, bool bSystem, bool bIndividual_Price, bool bOrder)
{
	return InvoicePrintOneRow(index, dr, bSystem, bIndividual_Price, false, bOrder);
}
string InvoicePrintOneRow(int index, DataRow dr, bool bSystem, bool bIndividual_Price, bool bKit, bool bOrder)
{
	int line_no = MyIntParse(index.ToString()) + 1;
	string code =dr["code"].ToString();
	string stock_locaiton = dr["stock_location"].ToString();
	string stock_qty = "";
	if(bOrder)
		stock_qty = dr["stock_qty"].ToString();
...
//	dNormalPrice = Math.Round((dNormalPrice/1.15),2);	 // Exclu G.S.T  
	...
	sb.Append("<td valign='top'>" + line_no + "</td>");
	if(bService)
	{
		sb.Append("<td>&nbsp;</td>");
		sb.Append("<td>&nbsp;</td>");
	...
	else
	{
		...
		if(bOrder)
		{
			sb.Append("<td valign=top>" + stock_qty + "</td>");
...
//DEBUG("m_inv_sNumber2",m_inv_sNumber);
	DataSet dsi = new DataSet();
	
	string sc = "SELECT  i.discount_percent, o.customer_gst, b.branch_header,  b.branch_footer, c2.name AS sales, o.system AS iSystem ";
	sc += ", o.no_individual_price AS iNoIndividual, o.paid AS Credit_Paid, o.*, c.*, i.code, cr.stock_location, sq.qty AS stock_qty ";
...
//		sc += ", (cr.price1 / 1.15) as normal_price ";
	...
	sc += " LEFT OUTER JOIN stock_qty sq ON sq.code = i.code AND sq.branch_id = " + Session["login_branch_id"].ToString();
